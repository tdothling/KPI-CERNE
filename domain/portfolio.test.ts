...
import {
  ...
  conjuntosKpis,
  revisaoStats,
  planPortfolioMigration,
  inferPapel,
  execMoldeKey,
  buildCodigoCompleto,
  portfolioToProjectFiles,
  PROJECAO_REF_PREFIX,
  PROJECAO_PRANCHA_PREFIX,
...
describe('rollupConjunto', () => {
  ...
  it('kpis de conjunto contam bases instanciadas/concluídas via rollup', () => {
    ...
    expect(conjuntosKpis([{ id: 'c1' }, { id: 'c2' }], pranchasByConjunto)).toEqual({
      bases: 2,
      concluidas: 1,
    });
  });
...
describe('unicidade referência+base', () => {
  it('segunda instanciação na mesma base falha', () => {
    const first = instanciarConjunto({
      referencia: refBSO,
      base: '104+000',
      existingConjuntos: [],
      today: '2026-07-08',
    ...
    expect(() =>
      instanciarConjunto({
        referencia: refBSO,
        base: '104+000',
        existingConjuntos: [first.conjunto],
        today: '2026-07-08',
      }),
...
describe('gerarReferenciasDisciplinas', () => {
  const arq: Referencia = {
    ...
    discipline: Discipline.ARCHITECTURE,
    gabarito: [
      { id: 'g1', papel: 'Folha 101' },
      { id: 'g2', papel: 'Folha 102' },
    ],
  };

  it('troca a sigla respeitando delimitadores (meio e fim do código)', () => {
    expect(swapDisciplinaSigla('BSO100-VIARAPOSOS-ARQ', 'ARQ', 'COB')).toBe(
      'BSO100-VIARAPOSOS-COB',
    );
    expect(swapDisciplinaSigla('BSO100-VIARAPOSOS-ARQ-R00', 'ARQ', 'HID')).toBe(
      'BSO100-VIARAPOSOS-HID-R00',
    );
  ...
  it('lote: gera para várias origens de uma vez, com resumo por origem', () => {
    const arqBSO = { ...arq };
    const arqPSP: Referencia = {
      ...arq,
      id: 'ref-arq-psp',
      codigoCliente: 'PSP200-VIARAPOSOS-ARQ',
    };
...
describe('planInstanciacaoLote', () => {
  const refPSP: Referencia = {
    ...refBSO,
    id: 'ref-psp',
    codigoCliente: 'PSP-CONSTRUCAP-EST-R00',
    gabarito: [
      { id: 'g1', papel: 'Folha 201' },
      { id: 'g2', papel: 'Folha 202' },
    ],
  };
  ...
  it('pula combinações já instanciadas em vez de duplicar', () => {
    const jaExiste = instanciarConjunto({
      referencia: refBSO,
      base: '104+000',
      existingConjuntos: [],
      today: '2026-07-08',
  ...
  it('referência sem gabarito é reportada e não gera conjunto vazio', () => {
    const semGab: Referencia = {
      ...refBSO,
      id: 'ref-vazia',
      codigoCliente: 'VAZIA-R00',
      gabarito: [],
    };
    const plan = planInstanciacaoLote({
...
describe('ciclos de vida independentes', () => {
  ...
  it('status da referência retroage/avança conforme as datas editadas', () => {
    ...
    expect(inferRefStatusFromDates({ startDate: '2026-05-01' }, RefStatus.ELABORADO)).toBe(
    ...
    ).toBe(RefStatus.ELABORADO);
    // Com envio → "Enviado"; apagar envio retroage para "Elaborado"
    expect(
      inferRefStatusFromDates({ startDate: 'a', endDate: 'b', sendDate: 'c' }, RefStatus.ELABORADO),
    ).toBe(RefStatus.ENVIADO);
    // Terminal preserva a decisão (Aprovado/Reprovado não é derivável de data)
    expect(
      inferRefStatusFromDates(
        { startDate: 'a', endDate: 'b', sendDate: 'c', feedbackDate: 'd' },
        RefStatus.APROVADO,
      ),
    ).toBe(RefStatus.APROVADO);
    expect(
      inferRefStatusFromDates(
        { startDate: 'a', endDate: 'b', sendDate: 'c', feedbackDate: 'd' },
        RefStatus.REPROVADO,
      ),
    ).toBe(RefStatus.REPROVADO);
    // Apagar o feedback tira do terminal e volta para "Enviado"
    expect(
      inferRefStatusFromDates({ startDate: 'a', endDate: 'b', sendDate: 'c' }, RefStatus.APROVADO),
    ).toBe(RefStatus.ENVIADO);
  });
  ...
  it('status da prancha retroage/avança conforme as datas editadas', () => {
    ...
    expect(inferPranchaStatusFromDates({ startDate: 'a' }, PranchaStatus.CONCLUIDO)).toBe(
      PranchaStatus.EM_ANDAMENTO,
    );
    expect(
      inferPranchaStatusFromDates({ startDate: 'a', endDate: 'b' }, PranchaStatus.EM_ANDAMENTO),
    ).toBe(PranchaStatus.CONCLUIDO);
    expect(
      inferPranchaStatusFromDates(
        { startDate: 'a', endDate: 'b', sendDate: 'c' },
        PranchaStatus.CONCLUIDO,
      ),
    ).toBe(PranchaStatus.ENVIADO);
    expect(
      inferPranchaStatusFromDates(
        { startDate: 'a', endDate: 'b', sendDate: 'c', feedbackDate: 'd' },
        PranchaStatus.REPROVADO,
      ),
    ).toBe(PranchaStatus.REPROVADO);
    expect(
      inferPranchaStatusFromDates(
        { startDate: 'a', endDate: 'b', sendDate: 'c' },
        PranchaStatus.REPROVADO,
  ...
  it('os KPIs contam apenas entidades comparáveis — nunca misturam os níveis', () => {
    const refs = [{ statusAprovacao: RefStatus.APROVADO }, { statusAprovacao: RefStatus.ENVIADO }];
    const pranchas = [
      { status: PranchaStatus.APROVADO },
      { status: PranchaStatus.APROVADO },
      { status: PranchaStatus.A_FAZER },
...
describe('revisaoStats', () => {
  const rev = (reason: RevisionReason, date = '2026-07-01') => ({
...
describe('planPortfolioMigration', () => {
  ...
  const prelim = makeProject({
    id: 'p1',
    filename: 'BSO-CONSTRUCAP-ARQ-R00',
  ...
  const execDocs = [
    makeProject({
      id: 'e1',
      filename: 'BSO-CONSTRUCAP-ARQ-R00_EXEC-101',
    ...
    makeProject({
      id: 'e2',
      filename: 'BSO-CONSTRUCAP-ARQ-R00_EXEC-102',
      phase: ProjectPhase.EXECUTIVE,
      base: '104+000',
    ...
    makeProject({
      id: 'e3',
      filename: 'BSO-CONSTRUCAP-ARQ-R00_EXEC-MD',
      phase: ProjectPhase.EXECUTIVE,
      base: '104+000',
      status: Status.IN_PROGRESS,
    }),
  ...
  it('preliminar vira referência; executivos viram conjuntos por base com suas pranchas', () => {
    ...
    expect(plan.pranchas.find((p) => p.conjuntoId === c090.id)!.status).toBe(
      PranchaStatus.REPROVADO,
    );
  ...
  it('é idempotente: IDs determinísticos já existentes são pulados', () => {
    const first = planPortfolioMigration({
      projects: [prelim, ...execDocs],
      rodoviaClients: ['Construcap 040RJ'],
      ...noExisting,
    });
    ...
    expect(second.skippedExisting).toBe(
      first.referencias.length + first.conjuntos.length + first.pranchas.length,
    );
  });

  it('só migra obras de rodovia — canteiros seguem o fluxo atual', () => {
    const canteiro = makeProject({
      id: 'c1',
      client: 'Canteiro Sede',
      phase: ProjectPhase.PRELIMINARY,
    });
    const plan = planPortfolioMigration({
...
describe('portfolioToProjectFiles', () => {
  ...
  it('projeta referência como Preliminar com status/datas equivalentes', () => {
    const ref: Referencia = {
      ...
      blockedDays: 3,
      revisions: [
        { id: 'r1', date: '2026-06-05', reason: RevisionReason.CLIENT_REQUEST, comment: '' },
      ],
      revisao: 1,
    };
  ...
  it('projeta prancha como Executivo herdando obra/base/disciplina do conjunto e da referência', () => {
    const p1 = prancha({
      ...
      endDate: '2026-06-05',
      sendDate: '2026-06-06',
      feedbackDate: '2026-06-09',
      blockedDays: 2,
      codigoCompleto: 'ELO-040RJ-104+000-BSO-EXE-DE-P1-101',
    });
  ...
  it('pranchas homônimas sem código, de conjuntos diferentes, não se fundem no mesmo entregável', () => {
    const conjunto090: Conjunto = {
      ...conjunto104,
      id: conjuntoIdFor(refBSO.id, '090+500'),
      base: '090+500',
    };
    const out = portfolioToProjectFiles(
      [refBSO],
  ...
  it('prancha órfã (conjunto excluído) e status sem equivalente ficam de fora', () => {
    const out = portfolioToProjectFiles(
      [],
      [conjunto104],
      [prancha({ conjuntoId: 'conjunto-inexistente' }), prancha({ status: PranchaStatus.A_FAZER })],
    );
    expect(out).toHaveLength(0);
  });
      ...
      revisions: [
        {
          id: 'rev1',
          date: '2026-07-02',
          reason: RevisionReason.SCOPE_CHANGE,
          comment: '',
          snapshot: {
            revisao: 0,
            startDate: '2026-06-01',
  ...
  it('entrada de revisão antiga (sem snapshot) não gera doc sintético', () => {
    const ref: Referencia = {
      ...
      startDate: '2026-07-02',
      revisions: [
        { id: 'rev1', date: '2026-07-02', reason: RevisionReason.CLIENT_REQUEST, comment: '' },
      ],
    };
    const out = portfolioToProjectFiles([ref], [], []);
    expect(out).toHaveLength(1);
...
describe('RefStatus.SUPERSEDED — integração com o restante do domínio', () => {
  ...
  it('editar datas não reabre o ciclo encerrado', () => {
    expect(inferRefStatusFromDates({ startDate: '2026-01-05' }, RefStatus.SUPERSEDED)).toBe(
      RefStatus.SUPERSEDED,
    );
    expect(inferRefStatusFromDates({}, RefStatus.SUPERSEDED)).toBe(RefStatus.SUPERSEDED);
    expect(
      inferRefStatusFromDates(
        { startDate: '2026-01-05', endDate: '2026-01-08', sendDate: '2026-01-09' },
        RefStatus.SUPERSEDED,
      ),
    ).toBe(RefStatus.SUPERSEDED);
  });
...
describe('refStatusAposDesinstanciar', () => {
  ...
  it('recua para a etapa que a linha do tempo indica', () => {
    ...
    expect(refStatusAposDesinstanciar({ ...base, startDate: '2026-01-05' })).toBe(
      RefStatus.EM_ELABORACAO,
    );
    expect(
      refStatusAposDesinstanciar({ ...base, startDate: '2026-01-05', endDate: '2026-01-08' }),
    ).toBe(RefStatus.ELABORADO);
    expect(
      refStatusAposDesinstanciar({
        ...base,
  ...
  it('com envio+feedback para em Enviado (o veredito é re-registrado pelos botões)', () => {
    expect(
      refStatusAposDesinstanciar({ ...base, sendDate: '2026-01-09', feedbackDate: '2026-01-15' }),
    ).toBe(RefStatus.ENVIADO);
  });

  it('referência que não estava Executivo Gerado passa intacta', () => {
    expect(refStatusAposDesinstanciar({ statusAprovacao: RefStatus.APROVADO })).toBe(
      RefStatus.APROVADO,
    );
    expect(
      refStatusAposDesinstanciar({
