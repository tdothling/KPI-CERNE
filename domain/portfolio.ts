...
export interface Prancha {
  ...
  startPeriod?: Period;
  endDate?: string;
  endPeriod?: Period; // fim da execução
  sendDate?: string;
  sendPeriod?: Period; // envio
  feedbackDate?: string;
  feedbackPeriod?: Period;
  blockedDays?: number;
...
export const refStatusAposDesinstanciar = (
  ref: Pick<Referencia, 'statusAprovacao' | 'startDate' | 'endDate' | 'sendDate' | 'feedbackDate'>,
): RefStatus => {
  ...
  return inferRefStatusFromDates(
    {
      ...
      endDate: ref.endDate,
      sendDate: ref.sendDate,
      feedbackDate: ref.feedbackDate,
    },
    RefStatus.ENVIADO,
  );
};
...
export const inferRefStatusFromDates = (dates: CycleDates, current: RefStatus): RefStatus => {
  ...
  if (sendDate && feedbackDate) {
    return current === RefStatus.APROVADO || current === RefStatus.REPROVADO
      ? current
      : RefStatus.ENVIADO;
  }
...
export const inferPranchaStatusFromDates = (
  dates: CycleDates,
  current: PranchaStatus,
): PranchaStatus => {
  const { startDate, endDate, sendDate, feedbackDate } = dates;
  if (sendDate && feedbackDate) {
    return current === PranchaStatus.APROVADO || current === PranchaStatus.REPROVADO
      ? current
...
export const buildCodigoCompleto = (
  codigoRodovia: string | undefined,
  item: GabaritoItem,
): string => {
...
export const instanciarConjunto = (input: InstanciarInput): InstanciarResult => {
  ...
  if (!referencia.gabarito || referencia.gabarito.length === 0) {
    throw new Error(
      'A referência não possui gabarito de entregáveis. Cadastre as saídas esperadas no Catálogo antes de instanciar.',
    );
  }
  ...
  if (existingConjuntos.some((c) => c.id === id)) {
    throw new Error(
      `A referência "${referencia.codigoCliente}" já foi instanciada na base "${base}".`,
    );
  }
...
export const swapDisciplinaSigla = (
  codigo: string,
  siglaOrigem: string,
  siglaDestino: string,
): string => {
...
export const gerarReferenciasDisciplinas = (
  input: GerarDisciplinasInput,
): GerarDisciplinasResult => {
  ...
  destinos.forEach(({ discipline, sigla }) => {
    ...
    result.novas.push({
      ...
      gabarito: origem.gabarito.map((g) => ({
        ...g,
        id: `gab-${slugify(codigo)}-${slugify(g.papel)}`,
      })),
...
export const planGerarDisciplinasLote = (
  input: GerarDisciplinasLoteInput,
): GerarDisciplinasLotePlan => {
...
export const planMoverPranchasLote = (
  pranchas: Prancha[],
  to: PranchaStatus,
): MoverPranchasLotePlan => {
  const plan: MoverPranchasLotePlan = { moviveis: [], puladas: [] };
  pranchas.forEach((p) =>
    (canPranchaTransition(p.status, to) ? plan.moviveis : plan.puladas).push(p),
  );
  return plan;
};
...
export const revisaoStats = (items: { revisions?: Revision[] }[]): RevisaoStats => {
  ...
  items.forEach((item) => {
    ...
    revs.forEach((r) => {
      stats.porMotivo[r.reason] = (stats.porMotivo[r.reason] || 0) + 1;
    });
  });
...
export const planPortfolioMigration = (input: MigrationInput): MigrationPlan => {
  ...
  const prelims = scoped.filter(
    (p) => (p.phase || ProjectPhase.PRELIMINARY) === ProjectPhase.PRELIMINARY,
  );
  ...
  const prelimFamilies = groupFamilies(prelims, (p) =>
    refKeyOf(p.client, p.discipline, cleanMoldeName(p.filename)),
  );
  ...
  const execFamilies = groupFamilies(
    execs,
    (p) =>
      `${normalizeClient(p.client)}|${p.discipline}|${normalizeClient(p.base || 'Geral')}|${p.groupId || slugify(cleanMoldeName(p.filename))}`,
  );
  ...
  conjuntoDrafts.forEach((draft) => {
    ...
    draft.pranchas.forEach(({ fam }, idx) => {
      const active =
        [...fam].reverse().find((p) => p.status !== Status.REVISED) || fam[fam.length - 1];
  ...
  plan.skippedExisting =
    before - (plan.referencias.length + plan.conjuntos.length + plan.pranchas.length);
