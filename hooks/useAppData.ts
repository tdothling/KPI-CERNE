...
import {
  ...
  subscribeToHolidays,
  saveHolidaysToDb,
  batchUpdateProjectsInDb,
  subscribeToSupplyOrders,
  addSupplyOrder,
  updateSupplyOrderInDb,
  deleteSupplyOrderFromDb,
  applySupplyStatusChange,
  patchSupplyOrderInDb,
  migrateLegacyPurchasesToSupply,
  subscribeToReferencias,
  ...
  instanciarLoteInDb,
  deleteConjuntoFromDb,
  patchPranchaInDb,
  batchUpdatePranchasInDb,
  addPranchaToDb,
  deletePranchaFromDb,
...
export function useAppData(projectFilter: ProjectFilterState) {
  ...
  useEffect(() => {
    ...
    referencias.forEach((r) => {
      ...
      if (fechado === r.statusAprovacao) {
        refSupersededSynced.current.delete(r.id);
        return;
      }
  ...
  const handleAddReferencias = async (
    ...
    for (let i = 0; i < refs.length; i++) {
      try {
        await addReferenciaToDb(refs[i]);
        criadas++;
      } catch (e: any) {
        erros.push(`${refs[i].codigoCliente}: ${e?.message || e}`);
      }
  ...
  const handleDeleteReferencias = async (
    ids: string[],
  ): Promise<{ excluidas: number; erros: string[] }> => {
    let excluidas = 0;
    const erros: string[] = [];
    for (const id of ids) {
      const ref = referencias.find((r) => r.id === id);
      try {
        await deleteReferenciaFromDb(id);
        excluidas++;
      } catch (e: any) {
        erros.push(`${ref?.codigoCliente || id}: ${e?.message || e}`);
      }
    }
  ...
  const handleUpdateReferencia = (id: string, changes: Partial<Referencia>) => {
    ...
    if (['sendDate', 'sendPeriod', 'feedbackDate', 'feedbackPeriod'].some((k) => k in changes)) {
      ...
      patch.blockedDays = recomputeBlockedDays(
        next.sendDate,
        next.sendPeriod,
        next.feedbackDate,
        next.feedbackPeriod,
    ...
    patchReferenciaInDb(id, patch).catch((e) =>
      alert('Erro ao salvar a referência: ' + (e?.message || e)),
    );
  };
  ...
  const handleDeleteReferencia = (id: string) => {
    ...
    if (vinculados > 0) {
      alert(
        `A referência "${ref.codigoCliente}" possui ${vinculados} conjunto(s) instanciado(s). Exclua os conjuntos na Carteira antes.`,
      );
      return;
    }
    if (!confirm(`Excluir a referência "${ref.codigoCliente}" do catálogo?`)) return;
    deleteReferenciaFromDb(id).catch((e) =>
      alert('Erro ao excluir a referência: ' + (e?.message || e)),
    );
  };
      ...
      if (ref.statusAprovacao === RefStatus.REPROVADO) {
        ...
        changes.revisions = [
          ...(ref.revisions || []),
          {
            ...
            date,
            reason: options.reason || RevisionReason.CLIENT_REQUEST,
            comment: options.comment || '',
            snapshot: buildCycleSnapshot(ref),
          },
        ];
      }
    }
  ...
  const handleDeleteConjunto = (conjunto: Conjunto) => {
    ...
    const comProgresso = doConjunto.filter(
      (p) => p.status !== PranchaStatus.A_FAZER || (p.revisions?.length ?? 0) > 0,
    );
    if (comProgresso.length === 0) {
      if (
        !confirm(
          `Excluir o conjunto da base "${conjunto.base}" e suas ${doConjunto.length} prancha(s)? Esta ação não pode ser desfeita.`,
        )
      )
        return;
    } else {
      const lista = comProgresso
        .slice(0, 10)
        .map((p) => `  • ${p.codigoCompleto || p.papel} — ${p.status}`)
        .join('\n');
    ...
    deleteConjuntoFromDb(conjunto.id).catch((e) =>
      alert('Erro ao excluir o conjunto: ' + (e?.message || e)),
    );
  };
  ...
  const buildPranchaMoveChanges = (
    ...
    if (to === PranchaStatus.CONCLUIDO) {
      changes.endDate = date;
      changes.endPeriod = period;
    }
    if (to === PranchaStatus.ENVIADO) {
      changes.sendDate = date;
      changes.sendPeriod = period;
    }
  ...
  const handleMovePrancha = (
    ...
    patchPranchaInDb(prancha.id, buildPranchaMoveChanges(prancha, to, date, period, options)).catch(
      (e) => alert('Erro ao mover a prancha: ' + (e?.message || e)),
    );
  };
  ...
  const handleMovePranchasLote = async (
    ...
    if (plan.moviveis.length === 0) return { movidas: 0, puladas: plan.puladas.length, erros: [] };
    const patches = plan.moviveis.map((p) => ({
      id: p.id,
      changes: buildPranchaMoveChanges(p, to, date, period, options),
    }));
    try {
  ...
  const handleUpdatePrancha = (id: string, changes: Partial<Prancha>) => {
    ...
    patchPranchaInDb(id, patch).catch((e) =>
      alert('Erro ao salvar a prancha: ' + (e?.message || e)),
    );
  };

  // Prancha extra fora do gabarito (ex.: detalhe específico de uma base)
  const handleAddPrancha = (prancha: Omit<Prancha, 'id'>) => {
    addPranchaToDb(prancha).catch((e) =>
      alert('Erro ao adicionar a prancha: ' + (e?.message || e)),
    );
  };

  const handleDeletePrancha = (prancha: Prancha) => {
    if (!confirm(`Excluir a prancha "${prancha.codigoCompleto || prancha.papel}"?`)) return;
    deletePranchaFromDb(prancha.id).catch((e) =>
      alert('Erro ao excluir a prancha: ' + (e?.message || e)),
    );
  };
  ...
  const handleMigratePortfolio = async () => {
    ...
    if (rodoviaClients.length === 0) {
      alert(
        'Nenhuma obra classificada como "Obra de Rodovia". Ajuste o tipo da obra na aba Obras antes de migrar.',
      );
      return;
    }
    try {
  ...
  const handleUpdateSupplyOrder = (order: SupplyOrder) => {
    updateSupplyOrderInDb(order).catch((e) =>
  ...
  const handleDeleteSupplyOrder = (id: string) => {
    if (confirm('Confirmar exclusão do pedido? Esta ação não pode ser desfeita.')) {
      deleteSupplyOrderFromDb(id).catch((e) =>
        alert('Erro ao excluir o pedido: ' + (e?.message || e)),
      );
    }
  };
  ...
  const handleMoveSupplyStatus = (
    ...
    applySupplyStatusChange(order.id, changes, event).catch((e) =>
      alert('Erro ao mover o pedido: ' + (e?.message || e)),
    );
  };
  ...
  const handleToggleSupplyItem = (orderId: string, itemId: string, delivered: boolean) => {
    ...
    const items = order.items.map((item) => {
      ...
      const { deliveredAt: _deliveredAt, ...rest } = item;
      return delivered
        ? { ...rest, delivered: true, deliveredAt: today }
        : { ...rest, delivered: false };
    });
    patchSupplyOrderInDb(orderId, { items }).catch((e) =>
      alert('Erro ao atualizar o item: ' + (e?.message || e)),
    );
  };
  ...
  const handleBatchUpdate = async (patches: ProjectBatchPatch[]) => {
    ...
    try {
      ...
      alert(
        'Erro ao aplicar a edição em lote. Nenhuma alteração parcial foi mantida — verifique sua conexão e tente novamente.',
      );
    }
  };
  ...
  const handleBatchWorkflow = async (
    ...
    ids.forEach((id) => {
      ...
      if (action === 'SEND') {
        changes.status = Status.WAITING_APPROVAL;
        changes.sendDate = date;
        changes.sendPeriod = period;
      }
    ...
    if (skipped > 0) {
      alert(
        `${skipped} arquivo(s) foram ignorados porque não estavam no status correto para a ação "${action}".`,
      );
    }
  };
  ...
  return {
    ...
    currentUser,
    trashItems,
    handleRestoreTrash: restoreFromTrash,
    handlePurgeTrash: purgeTrashEntries,
    updateProject,
    deleteProject,
    addProjectRevision,
