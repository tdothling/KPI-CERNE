...
export function useAppFilters(
    ...
    switch (dateFilterType) {
      ...
      case 'SEMESTER': {
        ...
        if (month < 6) {
          ...
          end = setDate(setMonth(referenceDate, 5), 30);
        } else {
          start = setDate(setMonth(referenceDate, 6), 1);
  ...
  const applyDateWindow = (list: ProjectFile[]) => {
    ...
    return list.filter((p) => {
      ...
      const projectStart = parseISO(p.startDate);
      const projectEnd =
        p.endDate && isValid(parseISO(p.endDate)) ? parseISO(p.endDate) : new Date();
      return projectStart <= filterEnd && projectEnd >= filterStart;
    });
  };
  ...
  const filteredSupplyOrders = useMemo(() => {
    ...
    if (dateRange) {
      ...
      result = result.filter((o) => {
        ...
        const orderEnd =
          deliveredAt && isValid(parseISO(deliveredAt)) ? parseISO(deliveredAt) : new Date();
  ...
  const uniqueClients = useMemo(() => {
    ...
    const merged = new Set([...registeredNames, ...projectClients, ...supplyClients]);
