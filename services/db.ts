...
const buildTrashEntry = (coll: string, docId: string, data: any, opId: string) =>
  sanitizeData({
    coll,
    docId,
    data,
...
const deleteWithTrash = async (coll: string, id: string) => {
  ...
  batch.set(
    doc(collection(db, COLL_TRASH)),
    buildTrashEntry(coll, id, snap.data(), crypto.randomUUID()),
  );
  batch.delete(ref);
  await batch.commit();
};
...
export const subscribeToProjects = (
  callback: (data: ProjectFile[]) => void,
...
export const addProject = async (project: Omit<ProjectFile, 'id'>) => {
  if (!isDbActive() || !checkAuth()) throw new Error('Acesso negado.');
  try {
    const { id: _id, ...cleanProject } = project as any;
...
export interface BatchDocPatch {
  id: string;
...
export const batchUpdateProjectsInDb = (patches: BatchDocPatch[]) =>
  batchUpdateCollection(COLL_PROJECTS, patches);

export const deleteProjectFromDb = async (id: string) => {
  if (!isDbActive() || !checkAuth()) throw new Error('Acesso negado.');
  try {
    await deleteWithTrash(COLL_PROJECTS, id);
  } catch (e) {
...
export const addSupplyOrder = async (order: Omit<SupplyOrder, 'id'>) => {
  if (!isDbActive() || !checkAuth()) throw new Error('Acesso negado.');
  try {
    const { id: _id, ...cleanOrder } = order as any;
    await addDoc(collection(db, COLL_SUPPLY), sanitizeData(cleanOrder));
  } catch (e) {
    console.error('Erro ao adicionar pedido de suprimentos:', e);
    throw e;
  }
};
...
export const updateSupplyOrderInDb = async (order: SupplyOrder) => {
  ...
  try {
    ...
    console.error('Erro ao atualizar pedido de suprimentos:', e);
    throw e;
  }
};
...
export const patchSupplyOrderInDb = async (id: string, changes: Record<string, any>) => {
  ...
  try {
    ...
    console.error('Erro ao mover pedido de suprimentos:', e);
    throw e;
  }
};
...
export const deleteSupplyOrderFromDb = async (id: string) => {
  if (!isDbActive() || !checkAuth()) throw new Error('Acesso negado.');
  try {
    await deleteWithTrash(COLL_SUPPLY, id);
  } catch (e) {
    console.error('Erro ao excluir pedido de suprimentos:', e);
    throw e;
  }
};
...
export const applySupplyStatusChange = async (
  id: string,
  changes: Record<string, any>,
  event: SupplyStatusEvent,
) => {
  if (!isDbActive() || !checkAuth()) throw new Error('Acesso negado.');
  try {
    await updateDoc(doc(db, COLL_SUPPLY, id), {
      ...sanitizeData(changes),
      statusHistory: arrayUnion(event),
    });
  } catch (e) {
    console.error('Erro ao registrar movimentação de suprimentos:', e);
    throw e;
  }
};
...
export const migrateLegacyPurchasesToSupply = async (
  userName: string,
    ...
    pending.slice(i, i + CHUNK).forEach((d) => {
      ...
      const order: Omit<SupplyOrder, 'id'> = {
        ...
        milestones: {
          ...
          ...(delivered && p.arrivalDate
            ? {
                deliveredAt: p.arrivalDate,
                ...(p.arrivalPeriod ? { deliveredPeriod: p.arrivalPeriod } : {}),
              }
            : {}),
        },
        statusHistory: [
...
const subscribeToCollection =
  <T>(collName: string, orderField: string, max: number) =>
...
export const subscribeToReferencias = subscribeToCollection<Referencia>(
  COLL_REFERENCIAS,
  'codigoCliente',
  1000,
);
export const subscribeToConjuntos = subscribeToCollection<Conjunto>(COLL_CONJUNTOS, 'base', 2000);
export const subscribeToPranchas = subscribeToCollection<Prancha>(
  COLL_PRANCHAS,
  'conjuntoId',
  5000,
);
...
export const deleteReferenciaFromDb = async (id: string) => {
  ...
  const linked = await getDocs(
    query(collection(db, COLL_CONJUNTOS), where('referenciaId', '==', id), limit(1)),
  );
  if (!linked.empty)
    throw new Error('Esta referência possui conjuntos instanciados. Exclua os conjuntos antes.');
...
export const batchUpdatePranchasInDb = (patches: BatchDocPatch[]) =>
  batchUpdateCollection(COLL_PRANCHAS, patches);
...
export const instanciarConjuntoInDb = async (
  conjunto: Conjunto,
  ...
  await runTransaction(db, async (tx) => {
    ...
    const { id: _id, ...conjuntoData } = conjunto;
...
export const deleteConjuntoFromDb = async (conjuntoId: string) => {
  if (!isDbActive() || !checkAuth()) throw new Error('Acesso negado.');
  const pranchasSnap = await getDocs(
    query(collection(db, COLL_PRANCHAS), where('conjuntoId', '==', conjuntoId)),
  );
  ...
  pranchasSnap.docs.forEach((d) => {
    batch.set(
      doc(collection(db, COLL_TRASH)),
      buildTrashEntry(COLL_PRANCHAS, d.id, d.data(), opId),
    );
    batch.delete(d.ref);
  });
  if (conjuntoSnap.exists()) {
    batch.set(
      doc(collection(db, COLL_TRASH)),
      buildTrashEntry(COLL_CONJUNTOS, conjuntoId, conjuntoSnap.data(), opId),
    );
  }
  batch.delete(doc(db, COLL_CONJUNTOS, conjuntoId));

  const referenciaId = conjuntoSnap.exists()
    ? ((conjuntoSnap.data() as any).referenciaId as string)
    : undefined;
  if (referenciaId) {
    const irmaosSnap = await getDocs(
      query(collection(db, COLL_CONJUNTOS), where('referenciaId', '==', referenciaId)),
    );
...
export const migrateProjectsToPortfolio = async (
  ...
  const projects = projectsSnap.docs.map(
    (d) => ({ ...(d.data() as any), id: d.id }) as ProjectFile,
  );
  ...
  for (let i = 0; i < writes.length; i += CHUNK) {
    ...
    writes
      .slice(i, i + CHUNK)
      .forEach((w) => batch.set(doc(db, w.coll, w.id), sanitizeData(w.data)));
    await batch.commit();
  }
...
export const addClient = async (client: Omit<ClientDoc, 'id'>) => {
  if (!isDbActive() || !checkAuth()) throw new Error('Acesso negado.');
  try {
    const { id: _id, ...cleanClient } = client as any;
...
export const updateClientInDb = async (client: ClientDoc) => {
  ...
  try {
    ...
    console.error('Erro ao atualizar cliente e vínculos:', e);
    throw e;
  }
};
...
export const deleteClientFromDb = async (id: string) => {
  if (!isDbActive() || !checkAuth()) throw new Error('Acesso negado.');
  try {
    await deleteWithTrash(COLL_CLIENTS, id);
  } catch (e) {
