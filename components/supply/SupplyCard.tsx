...
export const SupplyCard: React.FC<SupplyCardProps> = ({
  ...
  return (
    <div
      ...
      onClick={() => {
        if (!isDragging) onOpenDetails(order);
      }}
    ...
    >
      ...
      {total > 0 && (
        <div className="mb-2.5">
          <div className="flex items-center justify-between text-[10px] text-slate-400 dark:text-slate-500 mb-1">
            ...
            <span
              className={done === total ? 'text-emerald-600 dark:text-emerald-400 font-bold' : ''}
            >
              {done}/{total}
            </span>
          </div>
          <div className="h-1.5 bg-slate-100 dark:bg-slate-700 rounded-full overflow-hidden">
            <div
              className={`h-full rounded-full transition-all ${done === total ? 'bg-emerald-500' : 'bg-brand-500'}`}
      ...
      <div className="flex items-center justify-between gap-2">
        <span
          className={`inline-flex px-1.5 py-0.5 rounded text-[9px] font-bold border uppercase ${getPriorityColor(order.priority || 'NORMAL')}`}
        >
