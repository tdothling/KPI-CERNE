...
export const SUPPLY_KANBAN_COLUMNS: SupplyStatus[] = [
  ...SUPPLY_STATUS_ORDER,
...
const MILESTONE_FIELDS: Partial<
  Record<
    SupplyStatus,
    { at: keyof SupplyOrder['milestones']; period?: keyof SupplyOrder['milestones'] }
  >
> = {
...
export const PRIORITY_WEIGHT: Record<SupplyPriority, number> = {
  URGENTE: 0,
  ALTA: 1,
  NORMAL: 2,
  BAIXA: 3,
...
export const getPriorityColor = (priority: SupplyPriority): string => {
  switch (priority) {
    case 'URGENTE':
      return 'text-rose-700 bg-rose-100 border-rose-200 dark:bg-rose-900/40 dark:border-rose-700 dark:text-rose-400';
    case 'ALTA':
      return 'text-orange-700 bg-orange-100 border-orange-200 dark:bg-orange-900/40 dark:border-orange-700 dark:text-orange-400';
    case 'NORMAL':
      return 'text-blue-700 bg-blue-50 border-blue-200 dark:bg-blue-900/30 dark:border-blue-800 dark:text-blue-400';
    case 'BAIXA':
      return 'text-slate-500 bg-slate-100 border-slate-200 dark:bg-slate-700/50 dark:border-slate-600 dark:text-slate-400';
  }
};
