...
const inputClass =
  'w-full border border-slate-300 dark:border-slate-600 bg-slate-50 dark:bg-slate-900 text-slate-900 dark:text-white rounded-lg px-3 py-2 text-sm focus:ring-brand-500 focus:border-brand-500';
...
export const SupplyOrderModal: React.FC<SupplyOrderModalProps> = ({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!title.trim() || !client) {
      alert('Título e Obra são obrigatórios.');
      return;
    }
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input
              ...
              placeholder="Ex: Infraestrutura elétrica Bloco B"
              autoFocus
            />
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input
                ...
                className={inputClass}
                placeholder="Ex: Base 01, Galpão A"
                disabled={!showBaseInput}
              />
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                className={inputClass}
                placeholder="Ex: Infraestrutura de rede"
              />
            </div>
            <div>
              <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">
          ...
          <div className="grid grid-cols-3 gap-4">
            ...
            <div>
              ...
              <select
                value={priority}
                onChange={(e) => setPriority(e.target.value as SupplyPriority)}
                className={inputClass}
              >
          ...
          <div className="p-4 bg-slate-50 dark:bg-slate-700/30 rounded-lg border border-slate-100 dark:border-slate-700">
            <div className="flex items-center justify-between mb-3">
              <span className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase">
                Itens do Pedido ({items.filter((i) => i.description.trim()).length})
              </span>
              <button
                type="button"
            ...
            <div className="space-y-2 max-h-56 overflow-y-auto custom-scrollbar pr-1">
              {items.map((item, idx) => (
                <div key={item.id} className="flex items-center gap-2">
                  <span className="text-[10px] text-slate-400 w-5 text-right flex-shrink-0">
                    {idx + 1}.
                  </span>
                  <input
                    type="text"
                  ...
                  <button
                    ...
                    className="p-1.5 text-slate-400 hover:text-rose-600 rounded hover:bg-rose-50 dark:hover:bg-rose-900/30 transition-colors disabled:opacity-30"
          ...
          <div className="flex justify-end space-x-3 pt-3 border-t border-slate-100 dark:border-slate-700">
            ...
            <button
              type="submit"
              className="px-6 py-2 bg-brand-700 hover:bg-brand-800 text-white rounded-lg font-semibold shadow-md transition-all"
            >
