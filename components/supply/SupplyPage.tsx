...
export const SupplyPage: React.FC<SupplyPageProps> = ({
  ...
  const [pendingMove, setPendingMove] = useState<{ order: SupplyOrder; to: SupplyStatus } | null>(
    null,
  );
  ...
  const handleOpenEdit = (order: SupplyOrder) => {
  ...
  const handleMigrate = async () => {
    ...
    if (
      !confirm(
        'Importar os registros do módulo antigo de Compras para Suprimentos?\n\nOs dados antigos não são alterados nem apagados, e registros já importados são ignorados.',
      )
    )
      return;
    setMigrating(true);
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400">
            Do planejamento da lista à entrega na obra — tudo em um só fluxo.
          </p>
        </div>
        {!readOnly && (
          <button
            onClick={handleOpenCreate}
            className="bg-brand-700 hover:bg-brand-800 text-white px-4 py-2 rounded-lg text-sm font-medium transition-colors flex items-center gap-2 shadow-sm self-start sm:self-auto"
          >
      ...
      {isAdmin && onMigrateLegacy && (
        <div className="mb-6 p-4 bg-orange-50 dark:bg-orange-900/20 border border-orange-200 dark:border-orange-900/40 rounded-xl flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <span className="text-sm font-semibold text-slate-800 dark:text-slate-200 block">
                Importar dados antigos do módulo de Compras
              </span>
          ...
          <button
            ...
            disabled={migrating}
            className="px-4 py-2 bg-orange-500 hover:bg-orange-600 disabled:opacity-60 text-white rounded-lg text-xs font-bold transition-colors flex items-center gap-2 flex-shrink-0 self-start sm:self-auto"
          >
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden">
        <div className="p-4 border-b border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-700/30 flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
          <div className="relative flex-1 max-w-md">
            ...
            <input
              type="text"
              placeholder="Buscar por título, obra, item ou solicitante..."
              value={search}
              onChange={(e) => setSearch(e.target.value)}
              className="w-full pl-10 pr-4 py-2 border border-slate-200 dark:border-slate-600 rounded-lg focus:outline-none focus:border-brand-500 bg-white dark:bg-slate-800 text-slate-900 dark:text-white text-sm"
            />
          </div>
          <div className="flex items-center gap-3 justify-between sm:justify-end flex-1">
            ...
            <div className="grid grid-cols-2 gap-1 p-1 bg-slate-100 dark:bg-slate-900/50 rounded-lg">
              <button
                ...
                className={`flex items-center justify-center gap-1.5 px-3 py-1.5 rounded-md text-xs font-semibold transition-all ${viewMode === 'KANBAN' ? 'bg-white dark:bg-slate-700 text-brand-700 dark:text-brand-400 shadow-sm' : 'text-slate-500 hover:text-slate-700 dark:hover:text-slate-300'}`}
              >
                <LayoutGrid size={14} /> Kanban
              </button>
              <button
                onClick={() => setViewMode('TABLE')}
                className={`flex items-center justify-center gap-1.5 px-3 py-1.5 rounded-md text-xs font-semibold transition-all ${viewMode === 'TABLE' ? 'bg-white dark:bg-slate-700 text-brand-700 dark:text-brand-400 shadow-sm' : 'text-slate-500 hover:text-slate-700 dark:hover:text-slate-300'}`}
              >
        ...
        {viewMode === 'KANBAN' ? (
          <div className="p-4">
            <SupplyKanban
              ...
              onOpenDetails={(o) => setDetailsOrderId(o.id)}
              readOnly={readOnly}
            />
          </div>
        ) : (
