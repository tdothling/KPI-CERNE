  ...
  return (
    ...
    >
      <div className="flex items-center justify-between px-3 py-2.5 border-b border-slate-200/70 dark:border-slate-700/70">
        ...
        <span className="text-[10px] font-bold text-slate-400 bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-600 rounded-full px-2 py-0.5">
          {orders.length}
        </span>
      </div>
...
export const SupplyKanban: React.FC<SupplyKanbanProps> = ({
  orders,
  onRequestMove,
  onOpenDetails,
  readOnly = false,
      ...
      <DragOverlay dropAnimation={null}>
        {activeOrder && (
          <div className="w-[254px]">
            <SupplyCard order={activeOrder} onOpenDetails={() => {}} isOverlay />
          </div>
        )}
      </DragOverlay>
    </DndContext>
