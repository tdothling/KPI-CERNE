...
export const SupplyStatusMoveModal: React.FC<SupplyStatusMoveModalProps> = ({
  order,
  to,
  onConfirm,
  onClose,
  ...
  const config = isCancel
    ? {
        ...
        text: `Confirma o cancelamento de "${order.title}"?`,
    ...
    : isDelivery
      ? {
          ...
          text: 'Quando o material chegou na obra/base?',
      ...
      : to === SupplyStatus.BOUGHT
        ? {
            ...
            text: 'Quando o pedido foi realizado no fornecedor?',
        ...
        : {
            ...
            title: `Mover para ${to}`,
            text: 'Confirme a data do movimento.',
      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl shadow-2xl max-w-sm w-full p-6 border dark:border-slate-700 animate-in fade-in zoom-in-95 duration-150">
        ...
        {!isCancel && (
          <div className="flex mb-4">
            <input
              ...
              className="w-full border border-slate-300 dark:border-slate-600 bg-slate-50 dark:bg-slate-900 text-slate-900 dark:text-white rounded-l-lg px-3 py-2 dark:[color-scheme:dark]"
            />
            <select
              value={period}
              onChange={(e) => setPeriod(e.target.value as Period)}
              className="ml-1 bg-slate-100 dark:bg-slate-700 border border-slate-300 dark:border-slate-600 rounded text-xs px-1 py-1.5 focus:outline-none focus:border-brand-500"
            >
        ...
        {isDelivery && pendingItems > 0 && (
          <label className="flex items-start gap-2 mb-4 p-3 bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-100 dark:border-emerald-900/40 rounded-lg cursor-pointer">
            <input
              type="checkbox"
              checked={deliverAllItems}
              onChange={(e) => setDeliverAllItems(e.target.checked)}
              className="mt-0.5 text-emerald-600 focus:ring-emerald-500 rounded"
            />
            <span className="text-xs text-slate-600 dark:text-slate-300">
              Marcar os <b>{pendingItems} itens pendentes</b> como entregues nesta data
            </span>
          </label>
        )}
        ...
        {isCancel && (
          <textarea
            ...
            className="w-full border border-slate-300 dark:border-slate-600 bg-slate-50 dark:bg-slate-900 text-slate-900 dark:text-white rounded-lg px-3 py-2 mb-4 h-20 resize-none text-sm"
          ...
          <button
            onClick={() =>
              onConfirm(date, period, {
                ...
                deliverAllItems: isDelivery ? deliverAllItems : undefined,
              })
            }
            className={`px-4 py-2 text-white rounded-lg shadow-md font-medium ${config.confirmClass}`}
          >
            {config.confirmLabel}
          </button>
        </div>
