...
export const PurchaseList: React.FC<PurchaseListProps> = ({ purchases, onAdd, onUpdate, onDelete, currentUser, holidays, readOnly = false }) => {
  ...
  const handleOpenModal = (purchase?: PurchaseDoc) => {
      if (purchase) { setEditingId(purchase.id); const { id, ...data } = purchase; setFormData(data); } else { setEditingId(null); setFormData({ description: '', client: '', base: '', application: '', requester: currentUser || '', requestDate: new Date().toISOString().split('T')[0], arrivalDate: '', status: PurchaseStatus.PENDING, link: '', observation: '' }); }
      setIsModalOpen(true);
  };
  ...
  const getLeadTime = (start: string, end: string) => { if (!start) return null; const startDate = parseISO(start); const endDate = end ? parseISO(end) : new Date(); return calculateBusinessDays(startDate, endDate, holidays); };
                    ...
                    <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
                        {filteredPurchases.map(p => {
                            ...
                            <tr key={p.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                                ...
                                <td className="px-6 py-4"><div className="flex flex-col gap-1"><div className="flex items-center justify-between text-xs text-slate-500 dark:text-slate-400"><span title="Data do Pedido">{formatDateDisplay(p.requestDate)}</span><ArrowRight size={10} className="text-slate-300" /><span title="Data de Chegada" className={p.arrivalDate ? 'text-emerald-600 dark:text-emerald-400 font-bold' : ''}>{p.arrivalDate ? formatDateDisplay(p.arrivalDate) : '...'}</span></div>{p.status === PurchaseStatus.DELIVERED && p.arrivalDate && <span className="text-[10px] bg-slate-100 dark:bg-slate-700 text-slate-500 px-1.5 py-0.5 rounded self-start">Lead Time: {leadTime} dias úteis</span>}{p.status !== PurchaseStatus.DELIVERED && p.status !== PurchaseStatus.CANCELED && <span className="text-[10px] text-amber-600 dark:text-amber-500 px-1.5 py-0.5 self-start flex items-center gap-1"><Clock size={10} /> Aguardando há {leadTime} dias</span>}</div></td>
                                ...
                                {!readOnly && (
                                    <td className="px-6 py-4 text-center">
                                        <div className="flex items-center justify-center gap-2">
                                            {p.status === PurchaseStatus.PENDING && <button onClick={() => setPendingBuy({ id: p.id, date: new Date().toISOString().split('T')[0] })} title="Registrar Compra" aria-label="Registrar Compra" className="p-1.5 bg-blue-50 text-blue-600 hover:bg-blue-100 rounded-md transition-colors border border-blue-200"><CreditCard size={16} /></button>}
                                            {p.status === PurchaseStatus.BOUGHT && <button onClick={() => setPendingDelivery({ id: p.id, date: new Date().toISOString().split('T')[0] })} title="Registrar Chegada (Entrega)" aria-label="Registrar Chegada" className="p-1.5 bg-emerald-50 text-emerald-600 hover:bg-emerald-100 rounded-md transition-colors border border-emerald-200"><CheckSquare size={16} /></button>}
                                        </div>
                                    </td>
                                )}
