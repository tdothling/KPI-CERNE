...
export const PurchaseList: React.FC<PurchaseListProps> = ({ purchases, onAdd, onUpdate, onDelete, currentUser, holidays }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
      ...
      // Lógica automática: Se status for ENTREGUE e não tiver data de chegada, coloca hoje.
      ...
      // Se não for ENTREGUE, limpa a data de chegada (opcional, mas mantém consistência)
    ...
    <div className="animate-in fade-in zoom-in-95 duration-200">
        <div className="flex items-center justify-between mb-6">
            ...
            <button 
                ...
                className="bg-brand-700 hover:bg-brand-800 text-white px-4 py-2 rounded-lg text-sm font-medium transition-colors flex items-center gap-2 shadow-sm"
            >
        ...
        <div className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden">
            ...
            <div className="p-4 border-b border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-700/30 flex items-center gap-4">
                <div className="relative flex-1 max-w-md">
                        ...
                        <input 
                        type="text" 
                        placeholder="Buscar por descrição, cliente ou base..." 
                        value={search}
                        ...
                        {filteredPurchases.map(p => {
                            ...
                            <tr key={p.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                                ...
                                <td className="px-6 py-4">
                                    <div className="flex flex-col">
                                        ...
                                        <div className="flex items-center gap-3 mt-1 text-xs text-slate-500 dark:text-slate-400">
                                            ...
                                            {p.link && (
                                                <a href={p.link} target="_blank" rel="noopener noreferrer" className="text-blue-500 hover:underline flex items-center gap-1">
                                                    <ExternalLink size={10} /> Link
                                                </a>
                                            )}
                                        </div>
                                        {p.observation && (
                                            <span className="text-xs text-slate-400 mt-1 italic max-w-[300px] truncate" title={p.observation}>Obs: {p.observation}</span>
                                        )}
                                    </div>
                                </td>
                                ...
                                <td className="px-6 py-4 text-slate-600 dark:text-slate-400">
                                    <span className="bg-slate-100 dark:bg-slate-700 px-2 py-1 rounded text-xs border border-slate-200 dark:border-slate-600">
                                        {p.application || '-'}
                                    </span>
                                </td>
                                ...
                                <td className="px-6 py-4">
                                    <div className="flex flex-col gap-1">
                                        <div className="flex items-center justify-between text-xs text-slate-500 dark:text-slate-400">
                                            <span title="Data do Pedido">{formatDateDisplay(p.requestDate)}</span>
                                            <ArrowRight size={10} className="text-slate-300" />
                                            <span title="Data de Chegada" className={p.arrivalDate ? 'text-emerald-600 dark:text-emerald-400 font-bold' : ''}>
                                                {p.arrivalDate ? formatDateDisplay(p.arrivalDate) : '...'}
                                            </span>
                                        </div>
                                        ...
                                        {p.status !== PurchaseStatus.DELIVERED && p.status !== PurchaseStatus.CANCELED && (
                                             <span className="text-[10px] text-amber-600 dark:text-amber-500 px-1.5 py-0.5 self-start flex items-center gap-1">
                                                <Clock size={10} /> Aguardando há {leadTime} dias
                                             </span>
                                        )}
                                    </div>
                                </td>
                                ...
                                <td className="px-6 py-4 text-right">
                                    <div className="flex items-center justify-end space-x-2">
                                        ...
                                        <button onClick={() => onDelete(p.id)} className="p-1.5 text-slate-400 hover:text-rose-600 rounded hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors">
                                            <Trash2 size={16} />
                                        </button>
                                    </div>
                                </td>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                        <div>
                            ...
                            <input 
                                ...
                                placeholder="Ex: Material elétrico para o quadro QGBT"
                                autoFocus
                            />
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                             ...
                             <div>
                                ...
                                <input 
                                    ...
                                    className="w-full border border-slate-300 dark:border-slate-600 bg-slate-50 dark:bg-slate-900 text-slate-900 dark:text-white rounded-lg px-3 py-2"
                                    placeholder="Ex: Obra A, Galpão 1"
                                />
                             </div>
                        </div>
                        ...
                        <div>
                            ...
                            <input 
                                ...
                                className="w-full border border-slate-300 dark:border-slate-600 bg-slate-50 dark:bg-slate-900 text-slate-900 dark:text-white rounded-lg px-3 py-2"
                                placeholder="Ex: Infraestrutura de rede, Manutenção predial..."
                            />
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                             ...
                             <div>
                                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Status Atual</label>
                                <select 
                                    value={formData.status}
                                    onChange={(e) => setFormData({...formData, status: e.target.value as PurchaseStatus})}
                                    className="w-full border border-slate-300 dark:border-slate-600 bg-slate-50 dark:bg-slate-900 text-slate-900 dark:text-white rounded-lg px-3 py-2"
                                >
                                    {Object.values(PurchaseStatus).map(s => <option key={s} value={s} className="dark:bg-slate-800">{s}</option>)}
                                </select>
                             </div>
                        </div>

                        <div className="p-4 bg-slate-50 dark:bg-slate-700/30 rounded-lg border border-slate-100 dark:border-slate-700">
                            <span className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase mb-2 block flex items-center gap-2">
                                <Calendar size={12} /> Controle de Datas
                            </span>
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input 
                                        ...
                                        className="w-full border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 text-slate-900 dark:text-white rounded-lg px-3 py-2 text-sm dark:[color-scheme:dark]"
                                    />
                                </div>
                                <div>
                                    ...
                                    <input 
                                        ...
                                        className="w-full border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 text-slate-900 dark:text-white rounded-lg px-3 py-2 text-sm dark:[color-scheme:dark]"
                                        placeholder="Se vazio, não chegou"
                                    />
                                </div>
                            </div>
                        </div>
                        ...
                        <div>
                            <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Link de Referência (Opcional)</label>
                            <input 
                                type="url" 
                                value={formData.link || ''}
                         ...
                         <div>
                            ...
                            <textarea 
                                ...
                                className="w-full border border-slate-300 dark:border-slate-600 bg-slate-50 dark:bg-slate-900 text-slate-900 dark:text-white rounded-lg px-3 py-2 h-20 resize-none"
                        ...
                        <div className="flex justify-end space-x-3 pt-2 border-t border-slate-100 dark:border-slate-700 mt-2">
                            ...
                            <button type="submit" className="px-6 py-2 bg-brand-700 hover:bg-brand-800 text-white rounded-lg font-semibold shadow-md transition-all">
                                {editingId ? 'Salvar Alterações' : 'Registrar Solicitação'}
                            </button>
                        </div>
                    </form>
                 </div>
