...
import { calculateNetExecutionDuration } from '../utils';
...
export const DrillDownModal: React.FC<DrillDownModalProps> = ({
  ...
  const miniStats = useMemo(() => {
    const withRevision = rows.filter(
      (p) => p.revisions?.length > 0 || p.status === Status.REJECTED,
    );
    ...
    return {
      ...
      iapr:
        rows.length > 0 ? Math.round(((rows.length - withRevision.length) / rows.length) * 100) : 0,
  ...
  const statusLabel = (p: ProjectFile) => {
    if (p.status === Status.APPROVED)
      return { icon: <CheckCircle2 size={12} className="text-emerald-500" />, text: 'Aprovado' };
    if (p.status === Status.REJECTED)
      return { icon: <XCircle size={12} className="text-rose-500" />, text: 'Reprovado' };
    if (p.status === Status.WAITING_APPROVAL)
      return { icon: <Clock size={12} className="text-amber-500" />, text: 'Aguardando' };
    if (p.status === Status.DONE)
      return { icon: <CheckCircle2 size={12} className="text-violet-500" />, text: 'Concluído' };
    return { icon: <Clock size={12} className="text-blue-500" />, text: 'Em Andamento' };
  };

  return (
    <div
      className="fixed inset-0 bg-black/60 z-[70] flex items-end md:items-center justify-center p-0 md:p-4 backdrop-blur-sm"
      onClick={onClose}
    >
      <div
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100 dark:border-slate-700 shrink-0">
          <div className="flex items-center gap-3">
            <span
              className={`w-3 h-3 rounded-full ${payload.discipline ? DISCIPLINE_BG[payload.discipline] : 'bg-[#6366f1]'}`}
            />
        ...
        <div className="grid grid-cols-3 gap-px bg-slate-100 dark:bg-slate-700 border-b border-slate-100 dark:border-slate-700 shrink-0">
          {[
            ...
            <div
              key={kpi.label}
              className="flex flex-col items-center py-3 bg-white dark:bg-slate-800"
            >
            ...
            <table className="w-full text-xs">
              <thead className="sticky top-0 bg-slate-50 dark:bg-slate-700/80 backdrop-blur-sm z-10">
                <tr className="text-left">
                  <th className="px-4 py-2.5 text-slate-500 dark:text-slate-400 font-semibold uppercase tracking-wide">
                    Arquivo
                  </th>
                  <th className="px-4 py-2.5 text-slate-500 dark:text-slate-400 font-semibold uppercase tracking-wide hidden md:table-cell">
                    Cliente
                  </th>
                  <th className="px-4 py-2.5 text-slate-500 dark:text-slate-400 font-semibold uppercase tracking-wide">
                    Status
                  </th>
                  <th className="px-4 py-2.5 text-slate-500 dark:text-slate-400 font-semibold uppercase tracking-wide hidden sm:table-cell">
                    Início
                  </th>
                  <th className="px-4 py-2.5 text-slate-500 dark:text-slate-400 font-semibold uppercase tracking-wide hidden sm:table-cell">
                    Sem revisão
              ...
              <tbody>
                {rows.map((p, i) => {
                  ...
                  return (
                    <tr
                      key={p.id}
                      className={`border-t border-slate-50 dark:border-slate-700/50 hover:bg-slate-50 dark:hover:bg-slate-700/30 transition-colors ${i % 2 === 0 ? '' : 'bg-slate-50/50 dark:bg-slate-800/50'}`}
                    >
                      <td className="px-4 py-2.5">
                        <span
                          className="font-medium text-slate-700 dark:text-slate-200 truncate block max-w-[200px]"
                          title={p.filename}
                        >
                          {p.filename}
                        </span>
                      </td>
                      <td className="px-4 py-2.5 text-slate-500 dark:text-slate-400 hidden md:table-cell truncate max-w-[120px]">
                        {p.client}
                      </td>
                      <td className="px-4 py-2.5">
                        <span
                          className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full border text-[10px] font-semibold ${getStatusColor(p.status)}`}
                        >
                          {icon} {text}
                        </span>
                      </td>
                      <td className="px-4 py-2.5 text-slate-500 dark:text-slate-400 hidden sm:table-cell">
                        {p.startDate && isValid(parseISO(p.startDate))
                          ? format(parseISO(p.startDate), 'dd/MM/yy', { locale: ptBR })
                          : '—'}
                      </td>
