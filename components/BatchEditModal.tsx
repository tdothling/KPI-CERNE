...
import { ProjectFile, Discipline, Period, ProjectPhase } from '../types';
import {
  X,
  Search,
  CheckSquare,
...
const ACTIONS: {
...
}[] = [
  {
    ...
    desc: 'Substitui o valor de um campo nos arquivos selecionados.',
    icon: <Pencil size={18} />,
    activeClasses: 'border-slate-500 bg-slate-50 dark:bg-slate-700/50',
    iconActiveClasses: 'bg-slate-200 text-slate-700 dark:bg-slate-600 dark:text-slate-200',
  },
  {
    key: 'COMPLETE',
    label: 'Concluir Execução',
    desc: 'Define status "Execução Concluída" e a data fim.',
    icon: <CheckCircle2 size={18} />,
    activeClasses: 'border-violet-500 bg-violet-50 dark:bg-violet-900/30',
    iconActiveClasses: 'bg-violet-200 text-violet-700 dark:bg-violet-800 dark:text-violet-300',
  },
  {
    key: 'SEND',
    label: 'Registrar Envio',
    desc: 'Define "Aguardando Aprovação". Requer execução concluída.',
    icon: <Send size={18} />,
    activeClasses: 'border-blue-500 bg-blue-50 dark:bg-blue-900/30',
    iconActiveClasses: 'bg-blue-200 text-blue-700 dark:bg-blue-800 dark:text-blue-300',
  },
  {
    key: 'APPROVE',
    label: 'Aprovar Projetos',
    desc: 'Define "Aprovado" e recalcula dias bloqueados. Requer envio anterior.',
    icon: <BadgeCheck size={18} />,
    activeClasses: 'border-emerald-500 bg-emerald-50 dark:bg-emerald-900/30',
    iconActiveClasses: 'bg-emerald-200 text-emerald-700 dark:bg-emerald-800 dark:text-emerald-300',
  },
  {
    key: 'REJECT',
    label: 'Reprovar Projetos',
    desc: 'Define "Reprovado" e recalcula dias bloqueados. Requer envio anterior.',
    icon: <ThumbsDown size={18} />,
    activeClasses: 'border-rose-500 bg-rose-50 dark:bg-rose-900/30',
    iconActiveClasses: 'bg-rose-200 text-rose-700 dark:bg-rose-800 dark:text-rose-300',
  },
];
...
export const BatchEditModal: React.FC<BatchEditModalProps> = ({
  ...
  const canApply =
    eligibleProjects.length > 0 && valueIsValid && (action === 'EDIT_FIELD' || !!workflowDate);
  ...
  const buildEditPatches = (): ProjectBatchPatch[] => {
    return eligibleProjects.map((project) => {
      ...
      if (fieldConfig.type === 'date') {
        ...
        if (
          (selectedField === 'sendDate' || selectedField === 'feedbackDate') &&
          merged.sendDate &&
          merged.feedbackDate
        ) {
          ...
          if (isValid(send) && isValid(feedback) && feedback >= send) {
            changes.blockedDays = calculateBusinessDaysWithHolidays(
              send,
              feedback,
              holidays,
              merged.sendPeriod || 'MANHA',
              merged.feedbackPeriod || 'TARDE',
            );
          }
        }
  ...
  const inputClasses =
    'w-full bg-slate-50 dark:bg-slate-900 border border-slate-300 dark:border-slate-600 rounded-lg p-3 text-slate-900 dark:text-white focus:ring-2 focus:ring-brand-500 focus:border-brand-500 outline-none transition-all';
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <div className="p-3 border-b border-slate-200 dark:border-slate-700 flex items-center gap-3 bg-white dark:bg-slate-800">
          <div className="relative flex-1">
            ...
            <input
              type="text"
              placeholder="Buscar arquivo, cliente ou base..."
              value={searchFilter}
          ...
          <button
            ...
            className="flex items-center space-x-2 text-sm text-slate-700 dark:text-slate-300 hover:text-brand-600 dark:hover:text-brand-400 font-medium transition-colors whitespace-nowrap"
          >
        ...
        <div className="flex-1 overflow-y-auto p-2 space-y-1 bg-slate-50 dark:bg-slate-900">
          {filteredProjects.length === 0 ? (
            ...
            filteredProjects.map((project) => (
              ...
              >
                <div
                  className={`mr-3 ${selectedIds.has(project.id) ? 'text-brand-600 dark:text-brand-400' : 'text-slate-300 dark:text-slate-600'}`}
                >
    ...
    <>
      <div className="flex-1 flex flex-col md:flex-row overflow-hidden">
        ...
        <div className="w-full md:w-1/2 p-6 border-r border-slate-200 dark:border-slate-700 overflow-y-auto">
          <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-3">
            O que você deseja fazer com os {selectedProjects.length} arquivos?
          </label>
          <div className="grid grid-cols-1 gap-2">
            {ACTIONS.map((opt) => (
              ...
              >
                <div
                  className={`p-2 rounded-full ${action === opt.key ? opt.iconActiveClasses : 'bg-slate-100 text-slate-400 dark:bg-slate-800'}`}
                >
                  {opt.icon}
                </div>
                <div>
                  <span
                    className={`block text-sm font-bold ${action === opt.key ? 'text-slate-800 dark:text-white' : 'text-slate-600 dark:text-slate-400'}`}
                  >
                    {opt.label}
                  </span>
            ...
            <div>
              ...
              <div className="flex gap-2">
                ...
                <select
                  ...
                  onChange={(e) => setWorkflowPeriod(e.target.value as Period)}
                  className="bg-slate-50 dark:bg-slate-900 border border-slate-300 dark:border-slate-600 rounded-lg p-3 text-slate-900 dark:text-white focus:ring-2 focus:ring-brand-500 outline-none"
                >
                  <option value="MANHA">Manhã</option>
                  <option value="TARDE">Tarde</option>
          ...
          <div className="rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-4 space-y-2">
            ...
            {action === 'EDIT_FIELD' ? (
              <p className="text-sm text-slate-700 dark:text-slate-300">
                ...
                <strong>{newValue.trim() === '' ? '(defina o novo valor)' : newValue}</strong> em{' '}
                <strong>{selectedProjects.length}</strong> arquivo(s).
                {fieldConfig.type === 'date' &&
                  ' O período e o status serão atualizados automaticamente.'}
              </p>
            ) : (
              <p className="text-sm text-slate-700 dark:text-slate-300">
                A ação <strong>{ACTIONS.find((a) => a.key === action)?.label}</strong> será aplicada
                em <strong>{eligibleProjects.length}</strong> de {selectedProjects.length}{' '}
            ...
            {action !== 'EDIT_FIELD' && ineligibleCount > 0 && (
              <div className="flex gap-2 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 p-2 rounded text-xs text-amber-700 dark:text-amber-400">
                ...
                <p>
                  {ineligibleCount} arquivo(s) não estão no status adequado para esta ação e serão
                  ignorados.
                </p>
              </div>
            )}
      ...
      <div className="px-6 py-4 border-t border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-700/50 flex justify-between items-center">
        <button
          ...
          className="px-4 py-2.5 text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-600 rounded-lg font-medium transition-colors flex items-center gap-2"
        >
          <ArrowLeft size={18} />
        ...
        <div className="px-6 py-4 border-b border-slate-200 dark:border-slate-700 flex justify-between items-center bg-slate-50 dark:bg-slate-700/50">
          <div>
            <h3 className="text-xl font-bold text-slate-800 dark:text-white">
              Edição em Lote — Projetos
            </h3>
            <p className="text-sm text-slate-500 dark:text-slate-400">
              {step === 'SELECT'
                ? 'Passo 1 de 2 — Selecione os arquivos'
                : `Passo 2 de 2 — Escolha a ação (${selectedProjects.length} selecionados)`}
            </p>
          </div>
          <button
            type="button"
