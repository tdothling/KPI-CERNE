...
export const execDaysOf = (
  ...
  if (item.endDate) {
    ...
    return {
      days: calculateBusinessDaysWithHolidays(
        start,
        end,
        holidays,
        item.startPeriod || 'MANHA',
        item.endPeriod || 'TARDE',
      ),
      running: false,
    };
  }
...
const CARD_STYLES: Record<
  CardColor,
  { active: string; iconActive: string; iconIdle: string; value: string }
> = {
  slate: {
    active:
    ...
    iconIdle: 'bg-slate-100 text-slate-500 dark:bg-slate-700 dark:text-slate-400',
  ...
  red: {
    active:
      'border-red-500 bg-red-50 dark:bg-red-900/30 dark:border-red-500 ring-1 ring-red-300 dark:ring-red-600',
    iconActive: 'bg-red-100 text-red-700 dark:bg-red-900/50 dark:text-red-300',
    iconIdle: 'bg-slate-100 text-slate-500 dark:bg-slate-700 dark:text-slate-400',
    value: 'text-red-600 dark:text-red-400',
  },
  violet: {
    active:
      'border-violet-400 bg-violet-50 dark:bg-violet-900/30 dark:border-violet-500 ring-1 ring-violet-300 dark:ring-violet-600',
    iconActive: 'bg-violet-100 text-violet-700 dark:bg-violet-900/50 dark:text-violet-300',
    iconIdle: 'bg-slate-100 text-slate-500 dark:bg-slate-700 dark:text-slate-400',
    value: 'text-violet-700 dark:text-violet-400',
  },
  cyan: {
    active:
      'border-cyan-400 bg-cyan-50 dark:bg-cyan-900/30 dark:border-cyan-500 ring-1 ring-cyan-300 dark:ring-cyan-600',
    iconActive: 'bg-cyan-100 text-cyan-700 dark:bg-cyan-900/50 dark:text-cyan-300',
    iconIdle: 'bg-slate-100 text-slate-500 dark:bg-slate-700 dark:text-slate-400',
...
export function KpiCard({
  ...
  return (
    ...
    >
      <span
        className={`flex-shrink-0 w-8 h-8 rounded-lg flex items-center justify-center ${active ? s.iconActive : s.iconIdle}`}
      >
        {icon}
      </span>
      <span className="flex flex-col min-w-0">
        <span
          className={`text-lg font-bold leading-none ${active ? s.value : 'text-slate-700 dark:text-slate-200'}`}
        >
          {value}
        </span>
        <span className="text-[10px] font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-tight truncate">
          {label}
        </span>
      </span>
    </button>
...
export const REF_STATUS_STYLE: Record<RefStatus, string> = {
  [RefStatus.RASCUNHO]:
    'text-slate-600 bg-slate-100 border-slate-300 dark:bg-slate-700/60 dark:border-slate-600 dark:text-slate-300',
  [RefStatus.EM_ELABORACAO]:
...
export function StatusBadge({
  ...
  return (
    <span
      title={title}
      className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold border whitespace-nowrap ${className}`}
    >
      {label}
    </span>
  );
...
export function TimelineDatesEditor({
  fields,
  value,
  onChange,
  note,
    ...
    <div className="bg-slate-50/60 dark:bg-slate-900/40 border border-slate-200 dark:border-slate-700 rounded-lg p-3">
      <div className="flex items-center gap-1.5 mb-1">
        ...
        <label className="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase">
          Linha do Tempo (correção)
        </label>
      </div>
      <p className="text-[11px] text-slate-400 dark:text-slate-500 mb-3">
        {note ||
          'Ajuste qualquer data do processo. Os dias úteis com o cliente são recalculados a partir de envio e feedback. Deixe em branco para limpar.'}
      </p>
  ...
  return (
    <div className="fixed inset-0 bg-black/60 dark:bg-black/80 z-50 flex items-center justify-center p-4 backdrop-blur-sm print:hidden">
      <div className="bg-white dark:bg-slate-800 rounded-xl shadow-2xl max-w-sm w-full p-6 border dark:border-slate-700 animate-in fade-in zoom-in-95 duration-150">
        ...
        {request.description && (
          <p className="text-sm text-slate-500 dark:text-slate-400 mb-4">{request.description}</p>
        )}
        ...
        <div className="flex justify-end gap-2">
          ...
          <button
            onClick={() => {
              if (date) {
                request.onConfirm(date, period, comment, reason);
                onClose();
              }
            }}
...
// --- Cabeçalho "Selecionar todas / Limpar" para listas de seleção em modais de lote ---

export function SelHeader({
  label,
  allSelected,
  onToggleAll,
  disabled,
...
export function RevisionHistoryModal({
  ...
  const [draft, setDraft] = useState<{ reason: RevisionReason; comment: string }>({
    reason: RevisionReason.CLIENT_REQUEST,
    comment: '',
  });
  ...
  const saveEdit = () => {
    ...
    onSave(
      revisions.map((r) =>
        r.id === editingId ? { ...r, reason: draft.reason, comment: draft.comment } : r,
      ),
    );
    setEditingId(null);
  };
  ...
  return (
    <div className="fixed inset-0 bg-black/60 dark:bg-black/80 z-50 flex items-center justify-center p-4 backdrop-blur-sm print:hidden">
      <div className="bg-white dark:bg-slate-800 rounded-xl shadow-2xl max-w-lg w-full p-6 border dark:border-slate-700 animate-in fade-in zoom-in-95 duration-150 max-h-[85vh] flex flex-col">
        ...
        <p className="text-xs text-slate-400 dark:text-slate-500 font-mono mb-4 truncate">
          {title}
        </p>
            ...
            return (
              ...
              >
                <div className="flex items-center gap-2 flex-wrap">
                  <span className="bg-amber-100 dark:bg-amber-900/40 text-amber-700 dark:text-amber-400 text-[10px] font-bold px-1.5 py-0.5 rounded">
                    R{String(numero).padStart(2, '0')}
                  </span>
                  <span className="text-xs text-slate-500 dark:text-slate-400">
                    {formatDateDisplay(rev.date)}
                  </span>
                  {!isEditing && (
                  ...
                  {!readOnly && !isEditing && (
                    <button
                      ...
                      title="Editar motivo/comentário"
                ...
                {isEditing ? (
                  <div className="mt-2 space-y-2">
                    <select
                      value={draft.reason}
                      onChange={(e) =>
                        setDraft((d) => ({ ...d, reason: e.target.value as RevisionReason }))
                      }
                    ...
                    <div className="flex justify-end gap-2">
                      <button
                        ...
                        className="px-3 py-1.5 text-xs text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-lg font-medium"
                      >
                        Cancelar
                      </button>
                      <button
                        onClick={saveEdit}
                        className="px-3 py-1.5 text-xs text-white bg-brand-700 hover:bg-brand-800 rounded-lg font-semibold"
                      >
                        Salvar
                      </button>
                    </div>
                ...
                {s && (
                  <div className="mt-2 pt-1.5 border-t border-slate-200 dark:border-slate-700 grid grid-cols-2 gap-x-3 gap-y-0.5 text-[11px] text-slate-500 dark:text-slate-400">
                    <span className="col-span-2 font-semibold text-slate-600 dark:text-slate-300">
                      Ciclo encerrado — R{String(s.revisao).padStart(2, '0')}
                    </span>
                    ...
                    {exec && (
                      <span className="flex items-center gap-1">
                        <Clock size={11} /> Execução: {exec.days} dia(s) útil(eis)
                      </span>
                    )}
                    {(s.blockedDays ?? 0) > 0 && (
                      <span>Com cliente: {s.blockedDays} dia(s) útil(eis)</span>
                    )}
                  </div>
                )}
              </div>
