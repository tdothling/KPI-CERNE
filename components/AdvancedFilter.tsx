...
import { X, Filter, RotateCcw, Search, CheckSquare, Square } from 'lucide-react';
    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
      <div className="bg-white dark:bg-slate-800 rounded-xl shadow-2xl max-w-lg w-full flex flex-col max-h-[90vh] border dark:border-slate-700 animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="p-6 overflow-y-auto flex-1 space-y-6">
          <div className="bg-blue-50 dark:bg-blue-900/20 p-3 rounded-lg border border-blue-100 dark:border-blue-900/30">
            <p className="text-xs text-blue-700 dark:text-blue-300">
              Otimização de Banco de Dados: Esta busca consulta diretamente o servidor, trazendo
              apenas o necessário e economizando leituras.
            </p>
          </div>
        ...
        <div className="p-4 border-t border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-700/50 rounded-b-xl flex justify-between items-center">
          <button
            onClick={handleReset}
            className="text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-200 text-sm flex items-center gap-1 font-medium px-2 py-1 rounded hover:bg-slate-200 dark:hover:bg-slate-600 transition-colors"
          >
          ...
          <div className="flex gap-2">
            ...
            <button
              onClick={handleApply}
              className="px-6 py-2 bg-brand-700 hover:bg-brand-800 text-white rounded-lg shadow-sm font-semibold text-sm transition-all flex items-center gap-2"
            >
