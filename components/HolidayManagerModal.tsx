...
import { X, Calendar, Plus, Trash2, AlertCircle, CalendarRange, ChevronDown, ChevronRight, CalendarDays } from 'lucide-react';
...
export const HolidayManagerModal: React.FC<HolidayManagerModalProps> = ({ holidays, onUpdateHolidays, onClose }) => {
  ...
  // State to track expanded years in the list
  ...
  const handleAdd = () => {
    ...
    if (mode === 'SINGLE') {
        ...
        try {
            ...
            datesToAdd = interval.map(date => format(date, 'yyyy-MM-dd'));
        } catch (error) {
            console.error(error);
            return;
        }
    ...
    // Filter duplicates (dates that already exist)
    const newUniqueDates = datesToAdd.filter(d => !holidays.includes(d));

    if (newUniqueDates.length === 0) {
      alert('Todas as datas selecionadas já estão cadastradas.');
      return;
    }
    ...
    // Auto expand the year of the added date(s)
    if (newUniqueDates.length > 0) {
        const yearToExpand = getYear(parseISO(newUniqueDates[0]));
        setExpandedYears(prev => ({...prev, [yearToExpand]: true}));
    }
        ...
        <div className="flex flex-col md:flex-row gap-6 h-full overflow-hidden">
            ...
            <div className="w-full md:w-1/3 flex flex-col gap-4 border-b md:border-b-0 md:border-r border-slate-100 dark:border-slate-700 pb-4 md:pb-0 md:pr-4 shrink-0 overflow-y-auto">
                ...
                <div className="flex bg-slate-100 dark:bg-slate-900 p-1 rounded-lg shrink-0">
                    <button 
                        ...
                        className={`flex-1 py-1.5 text-xs font-bold uppercase tracking-wide rounded-md transition-all flex items-center justify-center gap-2 ${mode === 'SINGLE' ? 'bg-white dark:bg-slate-700 text-brand-700 dark:text-brand-300 shadow-sm' : 'text-slate-500 hover:text-slate-700 dark:hover:text-slate-300'}`}
                    >
                        <Calendar size={14} /> Data Única
                    </button>
                    <button 
                        onClick={() => setMode('RANGE')}
                        className={`flex-1 py-1.5 text-xs font-bold uppercase tracking-wide rounded-md transition-all flex items-center justify-center gap-2 ${mode === 'RANGE' ? 'bg-white dark:bg-slate-700 text-brand-700 dark:text-brand-300 shadow-sm' : 'text-slate-500 hover:text-slate-700 dark:hover:text-slate-300'}`}
                    >
                ...
                <div className="flex flex-col gap-4">
                    {mode === 'SINGLE' ? (
                        ...
                        <div className="grid grid-cols-1 gap-3">
                            <div>
                                <label className="block text-xs font-semibold text-slate-500 dark:text-slate-400 mb-1 uppercase">De</label>
                                <input 
                                    type="date" 
                                    value={rangeStart}
                                    onChange={(e) => setRangeStart(e.target.value)}
                                    className="w-full border border-slate-300 dark:border-slate-600 bg-slate-50 dark:bg-slate-900 text-slate-900 dark:text-white rounded-lg px-3 py-2 outline-none focus:ring-2 focus:ring-brand-500 text-sm dark:[color-scheme:dark]"
                                />
                            </div>
                            <div>
                                <label className="block text-xs font-semibold text-slate-500 dark:text-slate-400 mb-1 uppercase">Até</label>
                                <input 
                                    type="date" 
                                    value={rangeEnd}
                                    min={rangeStart}
                    ...
                    <button 
                        ...
                        className="w-full h-[40px] bg-brand-700 hover:bg-brand-800 disabled:bg-slate-300 dark:disabled:bg-slate-700 text-white rounded-lg transition-colors flex items-center justify-center shadow-sm font-semibold text-sm gap-2"
                    >
                        <Plus size={18} /> Adicionar
                    </button>
                </div>

                <div className="mt-auto pt-4 border-t border-slate-100 dark:border-slate-700 text-xs text-slate-400 flex items-start gap-2">
                    <AlertCircle size={14} className="flex-shrink-0 mt-0.5" />
                    <p>Datas cadastradas são subtraídas dos cálculos de dias úteis (apenas se caírem em dias de semana).</p>
                </div>
            </div>
            ...
            <div className="w-full md:w-2/3 flex flex-col h-full overflow-hidden">
                 <h4 className="text-sm font-bold text-slate-700 dark:text-slate-300 mb-3 flex items-center gap-2">
                    ...
                    Calendário ({holidays.length} dias)
                 </h4>

                <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar space-y-3">
                    {holidays.length === 0 ? (
                        <div className="text-center py-12 border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-lg h-full flex flex-col items-center justify-center">
                        ...
                        sortedYears.map(year => (
                            <div key={year} className="border border-slate-200 dark:border-slate-700 rounded-lg overflow-hidden bg-slate-50 dark:bg-slate-800/50">
                                <button 
                                    ...
                                    className="w-full flex items-center justify-between p-3 bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-700/80 transition-colors border-b border-slate-100 dark:border-slate-700"
                                >
                                    ...
                                    <div className="flex items-center gap-2">
                                        <span className="text-xs text-slate-500 bg-slate-100 dark:bg-slate-700 px-2 py-0.5 rounded-full">
                                            {Object.values(groupedHolidays[year]).flat().length} dias
                                        </span>
                                        {expandedYears[year] ? <ChevronDown size={18} className="text-slate-400" /> : <ChevronRight size={18} className="text-slate-400" />}
                                    </div>
                                </button>
                                        ...
                                        {Object.keys(groupedHolidays[year]).map(Number).sort((a, b) => a - b).map(month => {
                                            ...
                                            return (
                                                <div key={month} className="bg-white dark:bg-slate-800/50 rounded border border-slate-100 dark:border-slate-700 p-2">
                                                    <h5 className="text-xs font-bold text-brand-600 dark:text-brand-400 uppercase mb-2 border-b border-slate-100 dark:border-slate-700 pb-1">
                                                        {monthName}
                                                    </h5>
                                                    <div className="flex flex-wrap gap-2">
                                                        {monthDates.map(dateStr => {
                                                            ...
                                                            return (
                                                                <div 
                                                                    key={dateStr} 
                                                                    className={`group flex items-center gap-1.5 px-2 py-1 rounded text-xs border transition-colors ${
                                                                        isWe 
                                                                    ...
                                                                    title={`${format(date, "EEEE", { locale: ptBR })}${isWe ? ' (Fim de Semana)' : ''}`}
                                                                >
                                                                    <span className="font-mono font-medium">{format(date, 'dd')}</span>
                                                                    <button 
                                                                        onClick={() => handleRemove(dateStr)}
                                                                        className="text-slate-400 hover:text-rose-500 transition-colors opacity-50 group-hover:opacity-100"
                                                                    >
                                                                        <X size={12} />
                                                                    </button>
                                                                </div>
