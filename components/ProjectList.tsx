...
export const ProjectList: React.FC<ProjectListProps> = ({ projects, onUpdate, onDelete, onAddRevision, holidays, readOnly = false }) => {
  ...
  const sortedProjects = useMemo(() => {
    ...
    filtered.forEach(p => { const baseName = getProjectBaseName(p.filename).toLowerCase(); if (!groups[baseName]) { groups[baseName] = []; } groups[baseName].push(p); });
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-white dark:bg-slate-800 rounded-xl shadow-2xl max-w-2xl w-full p-6 border dark:border-slate-700 flex flex-col max-h-[90vh] overflow-y-auto">
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div><label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Data Início</label><input type="date" value={editingProject.startDate} onChange={(e) => updateEditingField('startDate', e.target.value)} className="w-full border border-slate-300 dark:border-slate-600 bg-slate-50 dark:bg-slate-900 text-slate-900 dark:text-white rounded-lg px-3 py-2 dark:[color-scheme:dark]" /></div>
                <div><label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Data Fim (Execução)</label><input type="date" value={editingProject.endDate} onChange={(e) => updateEditingField('endDate', e.target.value)} className="w-full border border-slate-300 dark:border-slate-600 bg-slate-50 dark:bg-slate-900 text-slate-900 dark:text-white rounded-lg px-3 py-2 dark:[color-scheme:dark]" /></div>
                <div className="md:col-span-2 grid grid-cols-3 gap-4 bg-slate-50 dark:bg-slate-700/30 p-4 rounded-lg border border-slate-200 dark:border-slate-700 mt-2"><div className="col-span-3 mb-1"><span className="text-xs font-bold text-brand-600 dark:text-brand-400 uppercase tracking-wide">Controle de Aprovação / Feedback</span></div><div><label className="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1">Envio</label><input type="date" value={editingProject.sendDate} onChange={(e) => updateEditingField('sendDate', e.target.value)} className="w-full border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 text-slate-900 dark:text-white rounded px-2 py-1.5 text-sm dark:[color-scheme:dark]" /></div><div><label className="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1">Feedback</label><input type="date" value={editingProject.feedbackDate} onChange={(e) => updateEditingField('feedbackDate', e.target.value)} className="w-full border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 text-slate-900 dark:text-white rounded px-2 py-1.5 text-sm dark:[color-scheme:dark]" /></div><div><label className="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1">Dias Bloq.</label><input type="number" step="0.5" value={editingProject.blockedDays} onChange={(e) => updateEditingField('blockedDays', e.target.value)} className="w-full border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 text-slate-900 dark:text-white rounded px-2 py-1.5 text-sm" /></div></div>
             </div>
