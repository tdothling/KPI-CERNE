...
import { Trash2, FileText, GitBranch, History, CornerDownRight, AlertTriangle, Edit2, Save, X, Eye, ArrowUpDown, ArrowUp, ArrowDown, BadgeCheck, Send, CheckSquare, XCircle, ThumbsDown, List, Search } from 'lucide-react';
...
// Helper to get the "clean" name of a project without revision tags or extensions
...
export const ProjectList: React.FC<ProjectListProps> = ({ projects, onUpdate, onDelete, onAddRevision, holidays }) => {
  ...
  const [pendingSend, setPendingSend] = useState<{ id: string, date: string } | null>(null);
  const [pendingApproval, setPendingApproval] = useState<{ id: string, date: string } | null>(null);
  const [pendingRejection, setPendingRejection] = useState<{ id: string, date: string } | null>(null);
  ...
  const projectHistory = useMemo(() => {
    if (!viewHistoryProject) return [];
    const baseName = getProjectBaseName(viewHistoryProject.filename);
    const lineage = projects.filter(p => getProjectBaseName(p.filename) === baseName);
    const historyEvents = lineage.flatMap(p => {
        const revNum = getRevisionNumber(p.filename);
        return p.revisions.map(rev => ({
            ...rev,
            fileId: p.id,
            filename: p.filename,
    ...
    return historyEvents.sort((a, b) => b.revisionNumber - a.revisionNumber);
  }, [viewHistoryProject, projects]);

  const detailsHistory = useMemo(() => {
    if (!detailsProject) return [];
    const baseName = getProjectBaseName(detailsProject.filename);
    const lineage = projects.filter(p => getProjectBaseName(p.filename) === baseName);
    const historyEvents = lineage.flatMap(p => {
        const revNum = getRevisionNumber(p.filename);
        return p.revisions.map(rev => ({
  ...
  const handleConfirmCompletion = () => {
    ...
    if (project) {
        if (project.startDate && pendingCompletion.date < project.startDate) {
        ...
        onUpdate({
            ...project,
            status: Status.DONE,
            endDate: pendingCompletion.date
        });
    }
  ...
  const handleConfirmSend = () => {
    ...
    if (project) {
        if (project.endDate && pendingSend.date < project.endDate) {
             alert("A Data de Envio não pode ser anterior à Data de Conclusão da execução.");
             return;
        }
        onUpdate({
            ...project,
            status: Status.WAITING_APPROVAL,
            sendDate: pendingSend.date
        });
    }
    setPendingSend(null);
  };
  ...
  const getStatusColor = (status: Status) => {
    switch(status) {
      case Status.APPROVED: return 'text-emerald-700 bg-emerald-100 border-emerald-300 dark:bg-emerald-900/40 dark:border-emerald-700 dark:text-emerald-400';
      case Status.REJECTED: return 'text-rose-700 bg-rose-100 border-rose-300 dark:bg-rose-900/40 dark:border-rose-700 dark:text-rose-400';
  ...
  const SortIcon = ({ column }: { column: SortKey }) => {
    if (sortConfig.key !== column) return <ArrowUpDown size={12} className="ml-1 text-slate-300 dark:text-slate-600 opacity-0 group-hover:opacity-100 transition-opacity" />;
    return sortConfig.direction === 'asc' 
      ? <ArrowUp size={12} className="ml-1 text-brand-600 dark:text-brand-400" />
      : <ArrowDown size={12} className="ml-1 text-brand-600 dark:text-brand-400" />;
  };

  const renderHeader = (label: string, key: SortKey, className: string = "") => (
    <th 
      className={`px-4 py-3 cursor-pointer group hover:bg-slate-100 dark:hover:bg-slate-700/50 transition-colors select-none ${className}`}
     ...
     return (
      <div className="animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="text-center py-20 bg-white dark:bg-slate-800 rounded-xl border border-dashed border-slate-300 dark:border-slate-700">
            ...
            <h3 className="text-lg text-slate-500 dark:text-slate-400">Nenhum projeto cadastrado</h3>
            <p className="text-sm text-slate-400 dark:text-slate-500">Importe arquivos ou adicione manualmente.</p>
        </div>
      </div>
    );
  }
    ...
    <div className="animate-in fade-in zoom-in-95 duration-200">
      ...
      <div className="p-4 border-b border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-700/30 flex items-center gap-4">
           ...
           <div className="text-sm text-slate-500 dark:text-slate-400">
             Total: <span className="font-semibold text-slate-800 dark:text-slate-200">{sortedProjects.length} registros</span>
           </div>
      </div>
        ...
        <table className="w-full text-sm text-left whitespace-nowrap">
          <thead className="text-xs text-slate-500 dark:text-slate-400 uppercase bg-slate-50 dark:bg-slate-700/50 border-b border-slate-200 dark:border-slate-700">
            <tr>
              ...
              {renderHeader("Fim Exec.", "endDate", "bg-slate-100/50 dark:bg-slate-800/50 border-r border-slate-200 dark:border-slate-700")}
              {renderHeader("Envio", "sendDate", "text-brand-700 dark:text-brand-400 bg-brand-50/50 dark:bg-brand-900/20 border-l border-brand-100 dark:border-brand-900/30")}
              {renderHeader("Feedback", "feedbackDate", "text-slate-700 dark:text-slate-300 bg-slate-50/50 dark:bg-slate-900/20 border-r border-slate-100 dark:border-slate-900/30")}
              {renderHeader("Bloqueado", "blockedDays", "w-24 text-center")}
              <th className="px-4 py-3 text-center">Ações de Fluxo</th>
              ...
              <tr key={project.id} className={`hover:bg-slate-50 dark:hover:bg-slate-700/30 transition-colors ${isLastInGroup ? 'border-b-4 border-slate-100 dark:border-slate-800' : ''}`}>
                <td className="px-4 py-3 font-medium text-slate-800 dark:text-slate-200 border-r border-slate-100 dark:border-slate-700/50">
                  <div className="flex items-center space-x-2 overflow-hidden" title={project.filename}>
                    ...
                    {isRevision ? (
                      <button onClick={() => setViewHistoryProject(project)} className="flex-shrink-0 bg-amber-100 dark:bg-amber-900/40 text-amber-700 dark:text-amber-400 text-[10px] font-bold px-1.5 py-0.5 rounded cursor-pointer hover:bg-amber-200 dark:hover:bg-amber-900/60 transition-colors">R{revNumber}</button>
                    ) : (
                      <span className="w-6"></span> 
                    )}
                    <span className={`truncate select-all ${isRevision ? 'text-slate-600 dark:text-slate-400 text-sm' : 'text-slate-800 dark:text-slate-200'}`}>
                      {project.filename}
                    </span>
                  </div>
                </td>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-white dark:bg-slate-800 rounded-xl shadow-2xl max-w-2xl w-full p-6 border dark:border-slate-700 flex flex-col max-h-[90vh] overflow-y-auto">
             <div className="flex justify-between items-center mb-6 border-b border-slate-100 dark:border-slate-700 pb-4">
                ...
                <button onClick={() => setEditingProject(null)} className="text-slate-400 hover:text-slate-600 dark:hover:text-slate-200"><X size={24} /></button>
             </div>
             <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="md:col-span-2">
                    <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Nome do Arquivo</label>
                    <input type="text" value={editingProject.filename} onChange={(e) => updateEditingField('filename', e.target.value)} className="w-full border border-slate-300 dark:border-slate-600 bg-slate-50 dark:bg-slate-900 text-slate-900 dark:text-white rounded-lg px-3 py-2" />
                </div>
                <div>
                    <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Cliente</label>
                    <input type="text" value={editingProject.client} onChange={(e) => updateEditingField('client', e.target.value)} className="w-full border border-slate-300 dark:border-slate-600 bg-slate-50 dark:bg-slate-900 text-slate-900 dark:text-white rounded-lg px-3 py-2" />
                </div>
                <div>
                    <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Base / Setor</label>
                    <input type="text" value={editingProject.base} onChange={(e) => updateEditingField('base', e.target.value)} className="w-full border border-slate-300 dark:border-slate-600 bg-slate-50 dark:bg-slate-900 text-slate-900 dark:text-white rounded-lg px-3 py-2" />
                </div>
                <div>
                   <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Disciplina</label>
                   <select value={editingProject.discipline} onChange={(e) => updateEditingField('discipline', e.target.value)} className="w-full border border-slate-300 dark:border-slate-600 bg-slate-50 dark:bg-slate-900 text-slate-900 dark:text-white rounded-lg px-3 py-2">
                ...
                <div>
                    ...
                    <input type="date" value={editingProject.startDate} onChange={(e) => updateEditingField('startDate', e.target.value)} className="w-full border border-slate-300 dark:border-slate-600 bg-slate-50 dark:bg-slate-900 text-slate-900 dark:text-white rounded-lg px-3 py-2 dark:[color-scheme:dark]" />
                </div>
                <div>
                    <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Data Fim (Execução)</label>
                    <input type="date" value={editingProject.endDate} onChange={(e) => updateEditingField('endDate', e.target.value)} className="w-full border border-slate-300 dark:border-slate-600 bg-slate-50 dark:bg-slate-900 text-slate-900 dark:text-white rounded-lg px-3 py-2 dark:[color-scheme:dark]" />
                </div>
                <div className="md:col-span-2 grid grid-cols-3 gap-4 bg-slate-50 dark:bg-slate-700/30 p-4 rounded-lg border border-slate-200 dark:border-slate-700 mt-2">
                    ...
                    <div><label className="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1">Envio</label><input type="date" value={editingProject.sendDate} onChange={(e) => updateEditingField('sendDate', e.target.value)} className="w-full border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 text-slate-900 dark:text-white rounded px-2 py-1.5 text-sm dark:[color-scheme:dark]" /></div>
                    <div><label className="block text-xs font-medium text-slate-500 dark:text-slate-400 mb-1">Feedback</label><input type="date" value={editingProject.feedbackDate} onChange={(e) => updateEditingField('feedbackDate', e.target.value)} className="w-full border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 text-slate-900 dark:text-white rounded px-2 py-1.5 text-sm dark:[color-scheme:dark]" /></div>
      ...
      {projectToDelete && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-white dark:bg-slate-800 rounded-xl shadow-2xl max-w-sm w-full p-6 border dark:border-slate-700">
            <div className="flex items-center space-x-3 mb-4">
              <div className="bg-rose-100 dark:bg-rose-900/30 p-2 rounded-full text-rose-600 dark:text-rose-400"><AlertTriangle size={24} /></div>
              <h3 className="text-lg font-bold text-slate-800 dark:text-white">Excluir Registro</h3>
            </div>
            <p className="text-sm text-slate-500 dark:text-slate-400 mb-6">Tem certeza que deseja excluir o arquivo <strong>{projectToDelete.filename}</strong>? Esta ação não pode ser desfeita.</p>
            <div className="flex justify-end space-x-3">
              <button onClick={() => setProjectToDelete(null)} className="px-4 py-2 text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-lg">Cancelar</button>
              <button onClick={handleConfirmDelete} className="px-4 py-2 bg-rose-600 hover:bg-rose-700 text-white rounded-lg shadow-md font-medium">Sim, Excluir</button>
            </div>
          </div>
        </div>
      )}
          ...
          <div className="bg-white dark:bg-slate-800 rounded-xl shadow-2xl max-w-lg w-full p-6 border dark:border-slate-700 flex flex-col max-h-[80vh]">
            <div className="flex justify-between items-start mb-4">
              <div>
                ...
                <p className="text-xs text-slate-500 dark:text-slate-400 mt-1 break-all">Projeto: <span className="font-semibold">{getProjectBaseName(viewHistoryProject.filename)}</span></p>
              </div>
              <button onClick={() => setViewHistoryProject(null)} className="text-slate-400 hover:text-slate-600 dark:hover:text-slate-200"><span className="text-2xl leading-none">&times;</span></button>
            </div>
            <div className="flex-1 overflow-y-auto pr-2">
              {projectHistory.length === 0 ? (
                <p className="text-sm text-slate-500 italic p-4 text-center border border-dashed rounded-lg">Nenhuma revisão registrada no histórico deste arquivo.</p>
              ) : (
                <div className="space-y-6 relative ml-2 pl-4 border-l-2 border-slate-200 dark:border-slate-700">
                  {projectHistory.map((rev, idx) => (
                    <div key={`${rev.fileId}-${idx}`} className="relative">
                       <div className="absolute -left-[23px] top-1 h-3 w-3 rounded-full bg-brand-600 dark:bg-brand-500 ring-4 ring-white dark:ring-slate-800"></div>
                       <div className="bg-slate-50 dark:bg-slate-700/50 rounded-lg p-3 border border-slate-100 dark:border-slate-700">
                           <div className="flex justify-between items-start mb-2">
                             <div className="flex items-center space-x-2">
                               <span className="text-xs font-bold bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-200 border border-slate-200 dark:border-slate-600 px-2 py-0.5 rounded shadow-sm">R{rev.revisionNumber}</span>
                               <span className="text-xs text-slate-500 dark:text-slate-400">{formatDateDisplay(rev.date)}</span>
                             </div>
                             <span className="text-xs font-semibold text-brand-700 dark:text-brand-400 bg-brand-50 dark:bg-brand-900/20 px-2 py-0.5 rounded">{rev.reason}</span>
                           </div>
                           <p className="text-sm text-slate-700 dark:text-slate-300 whitespace-pre-wrap leading-relaxed">{rev.comment}</p>
                       </div>
                    </div>
                  ))}
                </div>
             ...
             <div className="mt-6 pt-4 border-t border-slate-100 dark:border-slate-700 flex justify-end">
              <button onClick={() => setViewHistoryProject(null)} className="px-4 py-2 bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 dark:hover:bg-slate-600 text-slate-700 dark:text-slate-200 rounded-lg text-sm font-medium transition-colors">Fechar</button>
            </div>
          </div>
        </div>
      )}
      ...
      {detailsProject && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
          <div className="bg-white dark:bg-slate-800 rounded-xl shadow-2xl max-w-4xl w-full p-0 border dark:border-slate-700 flex flex-col max-h-[90vh] overflow-hidden">
             ...
             <div className="p-6 border-b border-slate-100 dark:border-slate-700 flex justify-between items-start bg-slate-50 dark:bg-slate-700/30">
                <div>
                   ...
                   <div className="flex items-center space-x-2 mt-2">
                      <span className={`px-2 py-0.5 rounded-full text-xs font-semibold border ${getStatusColor(detailsProject.status)}`}>{detailsProject.status}</span>
                      <span className="text-xs text-slate-400 dark:text-slate-500">ID: {detailsProject.id}</span>
                   </div>
                </div>
             ...
             <div className="overflow-y-auto p-6 space-y-8">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                   <div className="p-4 bg-slate-50 dark:bg-slate-700/40 rounded-lg border border-slate-100 dark:border-slate-700/50">
                      ...
                      <span className="text-lg font-semibold text-slate-800 dark:text-slate-100">{detailsProject.client}</span>
                   </div>
                   <div className="p-4 bg-slate-50 dark:bg-slate-700/40 rounded-lg border border-slate-100 dark:border-slate-700/50">
                      <span className="text-xs font-medium text-slate-500 dark:text-slate-400 uppercase block mb-1">Base</span>
                      <span className="text-lg font-semibold text-slate-800 dark:text-slate-100">{detailsProject.base || '-'}</span>
                   </div>
                   <div className="p-4 bg-slate-50 dark:bg-slate-700/40 rounded-lg border border-slate-100 dark:border-slate-700/50">
                      <span className="text-xs font-medium text-slate-500 dark:text-slate-400 uppercase block mb-1">Disciplina</span>
                      <span className="text-lg font-semibold text-slate-800 dark:text-slate-100">{detailsProject.discipline}</span>
                   </div>
                   <div className="p-4 bg-slate-50 dark:bg-slate-700/40 rounded-lg border border-slate-100 dark:border-slate-700/50">
                      <span className="text-xs font-medium text-slate-500 dark:text-slate-400 uppercase block mb-1">Dias Bloqueados</span>
                      <span className="text-lg font-semibold text-slate-800 dark:text-slate-100">{detailsProject.blockedDays} dias</span>
                   </div>
                   <div className="p-4 bg-slate-50 dark:bg-slate-700/40 rounded-lg border border-slate-100 dark:border-slate-700/50">
                      <span className="text-xs font-medium text-slate-500 dark:text-slate-400 uppercase block mb-1">Versão Atual</span>
                ...
                <div>
                   ...
                   <div className="flex flex-col md:flex-row justify-between items-center md:items-start bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl p-6 relative overflow-hidden">
                      <div className="hidden md:block absolute top-[45px] left-10 right-10 h-0.5 bg-slate-100 dark:bg-slate-700 z-0"></div>
                      <div className="relative z-10 flex flex-col items-center text-center w-full md:w-1/4 mb-4 md:mb-0">
                         <div className="w-8 h-8 rounded-full bg-slate-100 dark:bg-slate-700 border-4 border-white dark:border-slate-800 flex items-center justify-center mb-2"><div className="w-2.5 h-2.5 rounded-full bg-slate-400 dark:bg-slate-500"></div></div>
                         <span className="text-xs text-slate-400 dark:text-slate-500 font-medium uppercase mb-1">Início</span>
                         <span className="text-sm font-bold text-slate-700 dark:text-slate-200">{formatDateDisplay(detailsProject.startDate)}</span>
                      </div>
                      <div className="relative z-10 flex flex-col items-center text-center w-full md:w-1/4">
                         <div className="w-8 h-8 rounded-full bg-slate-100 dark:bg-slate-700 border-4 border-white dark:border-slate-800 flex items-center justify-center mb-2"><div className={`w-2.5 h-2.5 rounded-full ${detailsProject.endDate ? 'bg-slate-800 dark:bg-slate-200' : 'bg-transparent border border-slate-400'}`}></div></div>
                         <span className="text-xs text-slate-400 dark:text-slate-500 font-medium uppercase mb-1">Fim Exec.</span>
                         <span className="text-sm font-bold text-slate-700 dark:text-slate-200">{formatDateDisplay(detailsProject.endDate)}</span>
                      </div>
                      <div className="relative z-10 flex flex-col items-center text-center w-full md:w-1/4 mb-4 md:mb-0">
                         <div className="w-8 h-8 rounded-full bg-brand-50 dark:bg-brand-900/30 border-4 border-white dark:border-slate-800 flex items-center justify-center mb-2"><div className="w-2.5 h-2.5 rounded-full bg-brand-500"></div></div>
                         <span className="text-xs text-brand-600 dark:text-brand-400 font-medium uppercase mb-1">Envio</span>
                         <span className="text-sm font-bold text-brand-700 dark:text-brand-300">{formatDateDisplay(detailsProject.sendDate)}</span>
                      </div>
                      <div className="relative z-10 flex flex-col items-center text-center w-full md:w-1/4 mb-4 md:mb-0">
                         <div className={`w-8 h-8 rounded-full border-4 border-white dark:border-slate-800 flex items-center justify-center mb-2 ${detailsProject.status === Status.REJECTED ? 'bg-rose-50 dark:bg-rose-900/30' : 'bg-emerald-50 dark:bg-emerald-900/30'}`}>
                             <div className={`w-2.5 h-2.5 rounded-full ${detailsProject.status === Status.REJECTED ? 'bg-rose-500' : 'bg-emerald-500'}`}></div>
                         </div>
                         <span className={`text-xs font-medium uppercase mb-1 ${detailsProject.status === Status.REJECTED ? 'text-rose-600 dark:text-rose-400' : 'text-emerald-600 dark:text-emerald-400'}`}>
                             {detailsProject.status === Status.REJECTED ? 'Reprovação' : 'Aprovação'}
                         </span>
                         <span className={`text-sm font-bold ${detailsProject.status === Status.REJECTED ? 'text-rose-700 dark:text-rose-300' : 'text-emerald-700 dark:text-emerald-300'}`}>
                             {formatDateDisplay(detailsProject.feedbackDate)}
                         </span>
                      </div>
                   </div>
                </div>
                ...
                <div>
                  ...
                  {detailsHistory.length === 0 ? (
                    <div className="p-8 text-center border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-xl bg-slate-50 dark:bg-slate-800/50"><History size={32} className="mx-auto text-slate-300 dark:text-slate-600 mb-2" /><p className="text-slate-500 dark:text-slate-400">Este projeto ainda não possui revisões registradas.</p></div>
                  ) : (
                    <div className="border border-slate-200 dark:border-slate-700 rounded-xl overflow-hidden">
                       <table className="w-full text-sm text-left">
                          <thead className="bg-slate-50 dark:bg-slate-700/50 text-slate-500 dark:text-slate-400 uppercase text-xs border-b border-slate-200 dark:border-slate-700">
                             <tr><th className="px-6 py-3">Rev</th><th className="px-6 py-3">Data</th><th className="px-6 py-3">Motivo</th><th className="px-6 py-3">Comentário</th><th className="px-6 py-3 text-right">Arquivo Origem</th></tr>
                          </thead>
                          <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
                             {detailsHistory.map((rev, idx) => (
                                <tr key={`${rev.fileId}-${idx}`} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                                   <td className="px-6 py-4 font-bold text-slate-800 dark:text-slate-200">R{rev.revisionNumber}</td>
                                   <td className="px-6 py-4 text-slate-600 dark:text-slate-400">{formatDateDisplay(rev.date)}</td>
                                   <td className="px-6 py-4"><span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-brand-100 dark:bg-brand-900/30 text-brand-800 dark:text-brand-300">{rev.reason}</span></td>
                                   <td className="px-6 py-4 text-slate-600 dark:text-slate-300 max-w-md whitespace-normal">{rev.comment}</td>
                                   <td className="px-6 py-4 text-right text-xs text-slate-400 font-mono">{rev.filename}</td>
                                </tr>
                             ))}
                          </tbody>
                       </table>
             ...
             <div className="p-6 bg-slate-50 dark:bg-slate-700/30 border-t border-slate-100 dark:border-slate-700 flex justify-end">
                <button onClick={() => setDetailsProject(null)} className="px-6 py-2 bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-600 text-slate-700 dark:text-slate-200 font-medium rounded-lg hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors shadow-sm">Fechar</button>
             </div>
          </div>
        </div>
      )}
