        ...
        <div className="flex justify-between items-start p-6 border-b border-slate-100 dark:border-slate-700">
          <div>
            ...
            <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
              Cliente:{' '}
              <span className="font-semibold text-slate-700 dark:text-slate-200">{client}</span>
              {base && (
                <>
                  {' '}
                  · Base:{' '}
              ...
              <span className="block text-xs mt-0.5">
                Nada foi salvo ainda — revise as linhas e confirme.
              </span>
            </p>
          </div>
          <button
        ...
        <div className="px-6 py-3 bg-slate-50 dark:bg-slate-700/30 border-b border-slate-100 dark:border-slate-700 flex flex-wrap items-end gap-3">
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) => setBulkDiscipline(e.target.value as any)}
              className="bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-600 text-slate-900 dark:text-white text-xs rounded px-2 py-1.5"
            >
          ...
          <div>
            ...
            <input
              ...
              className="bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-600 text-slate-900 dark:text-white text-xs rounded px-2 py-1.5 dark:[color-scheme:dark]"
            />
          </div>
          <button
            onClick={applyBulk}
            disabled={saving}
            className="px-3 py-1.5 bg-brand-100 text-brand-700 hover:bg-brand-200 dark:bg-brand-900/30 dark:text-brand-400 rounded text-xs font-semibold transition-colors"
          >
            Aplicar
          </button>
        </div>
                ...
                >
                  ...
                  <td className="px-3 py-2">
                    <select
                      ...
                      onChange={(e) => updateRow(row.tempId, 'discipline', e.target.value)}
                      className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-600 text-slate-700 dark:text-slate-300 text-xs rounded px-2 py-1.5 w-full"
                    >
                      {Object.values(Discipline).map((d) => (
                        <option key={d} value={d}>
                          {d}
                  ...
                  <td className="px-3 py-2">
                    <select
                      value={row.phase}
                      onChange={(e) => updateRow(row.tempId, 'phase', e.target.value)}
                      className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-600 text-slate-700 dark:text-slate-300 text-xs rounded px-2 py-1.5 w-full"
                    >
                      {Object.values(ProjectPhase).map((p) => (
                        <option key={p} value={p}>
                  ...
                  <td className="px-3 py-2">
                    <div className="flex items-center gap-1">
                      <input
                        ...
                        className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-600 text-slate-700 dark:text-slate-300 text-xs rounded px-2 py-1.5 dark:[color-scheme:dark]"
                      />
                      <select
                        value={row.startPeriod}
                        onChange={(e) => updateRow(row.tempId, 'startPeriod', e.target.value)}
                        className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-600 text-slate-700 dark:text-slate-300 text-xs rounded px-1 py-1.5"
                      >
                  ...
                  <td className="px-3 py-2 text-right">
                    <button
                      ...
                      disabled={saving}
                      className="p-1.5 text-slate-400 hover:text-rose-500 rounded-full hover:bg-rose-50 dark:hover:bg-rose-900/30 transition-colors"
              ...
              {rows.length === 0 && (
                <tr>
                  <td colSpan={6} className="px-6 py-10 text-center text-slate-400 italic">
                    Nenhuma linha para cadastrar. Feche e tente novamente.
                  </td>
                </tr>
              )}
            </tbody>
        ...
        <div className="p-4 border-t border-slate-100 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-700/20 flex items-center justify-between gap-3">
          <span className="text-xs text-slate-500 dark:text-slate-400">
            {hasInvalidRow ? (
              <span className="text-rose-500 font-semibold">
                Há linhas sem nome — preencha ou remova para continuar.
              </span>
            ) : (
              'Todos os projetos entrarão como "Em Andamento".'
            )}
          </span>
          <div className="flex items-center gap-3">
            ...
            <button
              onClick={onConfirm}
              disabled={!canConfirm}
              className="px-6 py-2 bg-brand-700 hover:bg-brand-800 disabled:bg-slate-300 dark:disabled:bg-slate-600 text-white rounded-lg shadow-md font-semibold transition-all flex items-center gap-2"
            >
              {saving ? (
