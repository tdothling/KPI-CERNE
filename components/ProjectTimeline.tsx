...
import { differenceInCalendarDays, differenceInBusinessDays, format, startOfDay, endOfDay, eachDayOfInterval, eachMonthOfInterval, min, max, parseISO, isValid, isWeekend, isWithinInterval, addDays, isSameDay } from 'date-fns';
import { ptBR } from 'date-fns/locale';
import { ChevronRight, ChevronDown, Layers, ZoomIn, X, Briefcase, CalendarClock, CalendarRange, CheckCircle2, Download, AlertTriangle, Flag, TrendingUp } from 'lucide-react';
import { calculateBusinessDaysWithHolidays, calculateNetExecutionDuration, calculateDeadlineDate, addBusinessDaysWithHolidays } from '../utils';
...
const ClientDetailGantt = ({ clientName, projects, holidays, clients, onClose }: { clientName: string, projects: ProjectFile[], holidays: string[], clients: ClientDoc[], onClose: () => void }) => {
    ...
    const { rows, days, totalDays, chartStart, contractDeadline, holidaySet } = useMemo(() => {
        ...
        if (validProjects.length === 0) return { rows: [], days: [], totalDays: 0, chartStart: new Date(), contractDeadline: null as Date | null, holidaySet: hSet };
        const allDates: Date[] = [];
        ...
        Object.keys(discMap).sort().forEach(disc => {
            ...
            files.forEach(f => {
                ...
                fileRows.push({ id: f.id, type: 'FILE', label: f.filename, revision: f.revision, discipline: f.discipline, status: f.status, start, end, duration, netDuration, plannedEnd, deadlineDays, isLate, daysLate, send, feedback, pauses: f.pauses || [], predecessorIds: f.predecessorIds || [] });
            });
            ...
            if (discDates.length > 0) {
                ...
                rowData.push({ id: `disc-${disc}`, type: 'DISCIPLINE', label: disc, discipline: disc, start: minD, end: maxD, duration, lateCount, children: fileRows });
            }
        });
        ...
        return { rows: rowData, days: axisDays, totalDays: gRange, chartStart: gStart, contractDeadline: cDeadline, holidaySet: hSet };
    ...
    const deadlineOffset = contractDeadline ? differenceInCalendarDays(contractDeadline, chartStart) : -1;
                    ...
                    <div>
                        ...
                        <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4 mt-1">
                            ...
                            <div className="flex items-center gap-3 text-[10px] md:text-xs">
                                ...
                                <div className="flex items-center gap-1"><div className="w-0.5 h-3 bg-red-500"></div><span className="text-slate-600 dark:text-slate-300 font-medium tracking-tight uppercase">Hoje</span></div>
                                {contractDeadline && <div className="flex items-center gap-1"><div className="h-3 border-l-2 border-dashed border-amber-500"></div><span className="text-slate-600 dark:text-slate-300 font-medium tracking-tight uppercase">Prazo Contratual ({format(contractDeadline, 'dd/MM/yy')})</span></div>}
                                ...
                                <div className="flex items-center gap-1"><div className="w-1 h-3 bg-red-500"></div><span className="text-slate-600 dark:text-slate-300 font-medium tracking-tight uppercase">Caminho Crítico</span></div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div style={{ width: `${chartWidth}px` }} className="relative min-h-full">
                        <div className="flex border-b border-slate-200 dark:border-slate-700 sticky top-0 bg-white dark:bg-slate-800 z-30 h-12 shadow-sm">
                            ...
                            <div className="flex-1 flex relative">
                                {days.map((day, idx) => { const isWe = isWeekend(day); const isHol = holidaySet.has(format(day, 'yyyy-MM-dd')); const isToday = isSameDay(day, new Date()); const isMonthStart = format(day, 'dd') === '01' || idx === 0; const showDayNumber = dayWidth >= 24 || (dayWidth >= 12 && day.getDay() === 1) || (dayWidth < 12 && format(day, 'dd') === '01'); return (<div key={idx} style={{ width: `${100 / totalDays}%` }} title={isHol ? 'Feriado' : undefined} className={`border-l border-slate-100 dark:border-slate-700/50 flex flex-col items-center justify-end pb-2 ${isToday ? 'bg-red-50 dark:bg-red-900/30' : isHol ? 'bg-rose-50 dark:bg-rose-900/20' : isWe ? 'bg-slate-50 dark:bg-slate-900/50' : ''}`}>{showDayNumber && (<span className={`text-[10px] font-bold ${isToday ? 'text-red-600 dark:text-red-400' : isHol ? 'text-rose-400 dark:text-rose-500' : 'text-slate-400 dark:text-slate-500'}`}>{format(day, 'dd')}</span>)}{isMonthStart && (<span className="absolute top-1 text-[9px] font-bold text-brand-700 dark:text-brand-400 bg-brand-50 dark:bg-brand-900/50 px-1 rounded z-10 whitespace-nowrap">{format(day, 'MMM/yy', { locale: ptBR })}</span>)}</div>); })}
                            </div>
                        </div>
                        <div className="absolute top-12 bottom-0 left-[130px] md:left-[350px] right-0 flex pointer-events-none z-0">{days.map((day, idx) => { const isWe = isWeekend(day); const isHol = holidaySet.has(format(day, 'yyyy-MM-dd')); return (<div key={idx} style={{ width: `${100 / totalDays}%` }} className={`border-l border-slate-100 dark:border-slate-700/30 h-full ${isHol ? 'bg-rose-50/60 dark:bg-rose-900/10' : isWe ? 'bg-slate-50/50 dark:bg-slate-900/30' : ''}`} />); })}</div>
                        <div className="relative z-10">
                            {visibleRows.map((row: any) => {
                                ...
                                return (
                                    <div key={row.id} className={`flex items-center h-10 border-b border-slate-50 dark:border-slate-700/30 hover:bg-slate-50 dark:hover:bg-slate-700/20 group transition-colors ${isDisc ? 'bg-slate-50 dark:bg-slate-800' : 'bg-white dark:bg-slate-800'} ${row.isCritical ? 'border-l-2 border-l-red-500' : ''}`}>
                                            ...
                                            {isDisc ? (
                                                <button onClick={() => toggleDiscipline(row.label)} className="flex items-center space-x-2 w-full text-left font-semibold text-slate-700 dark:text-slate-200">
                                                    ...
                                                    <span className="ml-auto flex items-center gap-1">
                                                        {row.lateCount > 0 && <span className="text-[9px] md:text-[10px] px-1.5 py-0.5 bg-red-100 dark:bg-red-900/50 text-red-700 dark:text-red-400 rounded-full font-bold flex items-center gap-0.5" title={`${row.lateCount} arquivo(s) além do prazo`}><AlertTriangle size={9} />{row.lateCount}</span>}
                                                        <span className="text-[9px] md:text-[10px] px-1.5 py-0.5 bg-slate-200 dark:bg-slate-600 rounded-full text-slate-600 dark:text-slate-300 hidden md:inline-block">{row.children.length} arq</span>
                                                    </span>
                                                </button>
                                            ) : (
                                                <div className="pl-4 md:pl-6 flex items-center gap-1.5 text-xs md:text-sm text-slate-600 dark:text-slate-400 w-full overflow-hidden">
                                                    ...
                                                    {row.predecessorViolations?.length > 0 && (
                                                        <span className="flex-shrink-0" title={`Iniciou antes do término de: ${row.predecessorViolations.join(', ')}`}>
                                        ...
                                        <div className="flex-1 relative h-full">
                                            ...
                                            >
                                                <span className="drop-shadow-md">{row.type === 'FILE' && row.netDuration !== undefined ? row.netDuration : row.duration} dias{row.isLate ? ` · +${row.daysLate}d` : ''}</span>
                                            </div>
                                            ...
                                            {showTodayLine && <div className="absolute top-0 bottom-0 w-px bg-red-500/70 z-[12] pointer-events-none" style={{ left: `${todayLeftPct}%` }} />}
                                            {showDeadlineLine && <div className="absolute top-0 bottom-0 border-l-2 border-dashed border-amber-500/60 z-[12] pointer-events-none" style={{ left: `${deadlineLeftPct}%` }} />}
                                        </div>
                                    </div>
                                );
                            })}
...
export const ProjectTimeline: React.FC<ProjectTimelineProps> = ({ projects, holidays, clients }) => {
    ...
    const clientStats = useMemo(() => {
        const groups: Record<string, { totalFiles: number, startDate: Date, endDate: Date, activeFiles: number, deliveredFiles: number, relevantFiles: number, deliveryDates: Date[] }> = {};
        const today = new Date();
        ...
        projects.forEach(p => {
            if (!groups[p.client]) {
                groups[p.client] = { totalFiles: 0, startDate: new Date(2100, 0, 1), endDate: new Date(1900, 0, 1), activeFiles: 0, deliveredFiles: 0, relevantFiles: 0, deliveryDates: [] };
            }
        ...
        return Object.entries(groups).map(([name, stats]) => {
            ...
            const progress = stats.relevantFiles > 0 ? Math.round((stats.deliveredFiles / stats.relevantFiles) * 100) : 0;
                    ...
                    >
                        ...
                        <div className="relative z-10">
                            ...
                            <div className="space-y-3">
                                ...
                                {client.deadline && (
                                    <div className={`flex items-center gap-2 text-sm ${client.isOverdue ? 'text-red-600 dark:text-red-400 font-semibold' : 'text-slate-600 dark:text-slate-400'}`}>
                                        <Flag size={16} className={client.isOverdue ? 'text-red-500' : 'text-brand-600 dark:text-brand-400'} />
